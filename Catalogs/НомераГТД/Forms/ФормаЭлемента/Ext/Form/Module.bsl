...
Функция ПроверитьУникальностьРНПТКомплекта(Отказ)
	...
	ТаблицаКомплектующих.Свернуть("НомерРНПТ, ЕдиницаИзмеренияТНВЭД", "КоличествоПоРНПТ");
...
Процедура ЗаполнитьПоПараметрамСоздания()
	...
	Если Параметры.Свойство("ПараметрыСоздания", ПараметрыСоздания)
		...
		Если ПараметрыСоздания.Свойство("ДанныеТовара")
			И ТипЗнч(ПараметрыСоздания.ДанныеТовара) = Тип("Структура")
			И ПараметрыСоздания.ДанныеТовара.Свойство("Номенклатура") Тогда
			
			Номенклатура = ПараметрыСоздания.ДанныеТовара.Номенклатура;
		...
		Если Не ЗначениеЗаполнено(Объект.СтранаПроисхождения)
			...
			Объект.СтранаПроисхождения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "СтранаПроисхождения");
