...
Процедура Использование_НачислениеРезервовПоОплатеТруда(ПараметрыОбработчика) Экспорт
	...
	УчетЗарплатыОтключен = НЕ ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты");
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РезервыПоОплатеТруда КАК РегламентныйДокумент
	...
	|ГДЕ
	|	&ИспользуютсяРезервыПоОплатеТруда
	|	И РегламентныйДокумент.Ссылка ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
