	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции) Тогда
		...
		Иначе
			ПараметрыЗапроса.Добавить(
				СтрШаблон(
					"prefix=%1",
					КодироватьСтроку("2402,2403,2404", СпособКодированияСтроки.КодировкаURL)));
		КонецЕсли;
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				Если ДанныеОбработки.Свойство("type") Тогда
					Если ДанныеОбработки.type = "EAS_GTIN_CROSSBORDER_IMPORT" Тогда
						ПараметрыОбработки.ИмяПоляСотировки = "gtin";
					ИначеЕсли ДанныеОбработки.type = "EAS_CROSSBORDER" Тогда
						ПараметрыОбработки.ИмяПоляСотировки = "cis";
					КонецЕсли;
				КонецЕсли;
