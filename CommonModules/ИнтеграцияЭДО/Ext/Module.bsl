...
Функция ОписаниеОбъектаУчета(ИсточникДанных) Экспорт
	...
	ИначеЕсли ТипИсточникаДанных = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		СтрокаОписания.Направление  = Перечисления.НаправленияЭДО.Исходящий;
		СтрокаОписания.Организация  = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсточникДанныхСсылка, "Организация");
		СтрокаОписания.Контрагент   = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсточникДанныхСсылка, "Владелец");
...
Функция ОписаниеДанныхОбъектаУчета(ОбъектУчета, Параметры) Экспорт
	...
	Иначе
		МенеджерФормата = ФорматыЭДО.НайтиМенеджерФормата(ТипДокумента, Формат);
		Если ТипЗнч(МенеджерФормата) = Тип("Неопределено") Тогда
			...
			ВызватьИсключение СтрокаСообщенияИсключения;
		Иначе
			Результат = МенеджерФормата.ОписаниеДанныхОбъектаУчета(ОбъектУчета, Параметры);
		КонецЕсли;
	КонецЕсли;
...
//
Функция ЗаполнитьАдресДляДоговорногоДокумента101(ВидКонтактнойИнформации, ОбъектКонтактнойИнформации, ДатаАдреса,
	...
	КонтактнаяИнформация = КонтактнаяИнформацияАдресаДляДоговорногоДокумента101(ВидКонтактнойИнформации,
		ОбъектКонтактнойИнформации, ДатаАдреса);
...
//
Функция ЗаполнитьКодСтраныИностранногоАдресаДляДоговорногоДокумента101(ВидКонтактнойИнформации,
		ОбъектКонтактнойИнформации, ДатаАдреса, КонструкторЭД) Экспорт
...
//
Функция КонтактнаяИнформацияАдресаДляДоговорногоДокумента101(ВидКонтактнойИнформации, ОбъектКонтактнойИнформации,
		ДатаАдреса) Экспорт
