...
Процедура ПолеАдресаАвтоПодбор(Форма, ВидДанных,
	...
	Если ВидДанных = "ЗемельныйУчасток" Тогда
		ДанныеВыбора = ИнтеграцияЗЕРНОВызовСервера.СписокАвтоподбораЗемельныйУчасток(ТекстДляАвтоПодбора);
	ИначеЕсли ВидДанных = "СкладКонтрагент" Тогда
		ДанныеВыбора = ИнтеграцияЗЕРНОВызовСервера.СписокАвтоподбораСкладКонтрагент(ТекстДляАвтоПодбора);
	КонецЕсли;
