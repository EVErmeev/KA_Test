...
Функция ПараметрыВыбораОбеспечения(Форма, ТекущаяСтрока, Режим) Экспорт
	...
	ФормаСтруктурой = Новый Структура();
	
	ДанныеФормыСтруктураСКоллекцией(
		Форма,
		ФормаСтруктурой,
		ПараметрыВстраивания.ШаблонСериализацииДанныхФормы);
	ФормаСтруктурой.Вставить("ДоступныеОстаткиПараметрыВстраивания",
		ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ПараметрыВстраивания));
	ФормаСтруктурой.Вставить("УникальныйИдентификатор", Форма.УникальныйИдентификатор);
	
	Если ПараметрыВстраивания.АдресКорректировкиДвижений <> Неопределено Тогда
		ФормаСтруктурой.Вставить(ПараметрыВстраивания.АдресКорректировкиДвижений,
		Форма[ПараметрыВстраивания.АдресКорректировкиДвижений]);
	КонецЕсли;
	
	ПараметрыФормы = ОбеспечениеВДокументахВызовСервера.ПараметрыВыбораОбеспечения(ФормаСтруктурой, ТекущаяСтрока, Режим);
...
Процедура ДанныеФормыСтруктураСКоллекцией(Форма, ФормаСтруктурой, ШаблонСериализации)
	
	Для Каждого Свойство Из ШаблонСериализации Цикл
		
		ФормаСтруктурой.Вставить(Свойство.Ключ);
		Если ТипЗнч(Свойство.Значение) = Тип("Структура") Тогда
			
			Если ТипЗнч(Форма[Свойство.Ключ]) = Тип("ДанныеФормыКоллекция") Тогда
				ФормаСтруктурой[Свойство.Ключ] = Форма[Свойство.Ключ];
			Иначе
				ФормаСтруктурой[Свойство.Ключ] = Новый Структура();
				ДанныеФормыСтруктураСКоллекцией(
					Форма[Свойство.Ключ],
					ФормаСтруктурой[Свойство.Ключ],
					Свойство.Значение);
			КонецЕсли;
