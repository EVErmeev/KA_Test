	...
	Для Каждого Макет Из Макеты Цикл // Структура
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(ОбъектШаблона) Тогда
				Продолжить;
			КонецЕсли;
			Область = ОбъектШаблона.РедакторТабличныйДокумент.Область("R1C1"); // ОбластьЯчеекТабличногоДокумента
			ШиринаСтолбца = Область.Примечание.Ширина;
			ВысотаСтроки = Область.Примечание.Высота;
			
			ЗаписьXML = Новый ЗаписьXML;
			ЗаписьXML.УстановитьСтроку();
			СериализаторXDTO.ЗаписатьXML(ЗаписьXML, ОбъектШаблона.РедакторТабличныйДокумент);
			...
			Для Каждого ПараметрМакета Из ТаблицаПараметров Цикл
				...
				Пока НачальныйРяд < ТекущийРяд Цикл
					ЗначениеФорматаРяда = СоответствиеКолонок.Получить(НачальныйРяд);
					ВысотаРяда = ?(ЗначениеЗаполнено(ЗначениеФорматаРяда), Число(СоответствиеФорматов.Получить(Число(ЗначениеФорматаРяда))), Число(СоответствиеФорматов.Получить(Число(ФорматПоУмолчанию))));
			...
			Окр(ВысотаМакета/КоличествоПикселейНаЕдиницуВысоты), 0, Поля);
