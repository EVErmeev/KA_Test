...
Процедура ДополнитьПараметрамиПоУмолчанию(Параметры) Экспорт
	...
	Если Параметры.Свойство("ТипОперации") И Параметры.ТипОперации = Перечисления.ТипыРегламентныхОпераций.ПризнаниеРасходовПриУСН Тогда
		...
		Параметры.Вставить("ТипыНДФЛВзносы", ИнтеграцияБЗК.ТипыНалоговНДФЛИВзносов());
...
Функция ТекстЗапросаВыборкиДокументовДляЗаполненияРасходов() Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаРегистраторов.ВидОперации В (""Оплата"", ""ОплатаБезРазбивкиПоДокументам"", ""Аванс"", ""АвансБезРазбивкиПоДокументам"")
	|		ТОГДА 1
	|		ИНАЧЕ 10
	|	КОНЕЦ * ВЫБОР
...
Функция ТекстЗапросаВыборкиДокументовДляЗаполненияСторно() Экспорт
	...
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ Т.Регистратор, Т.Организация, Т.ДоговорЭквайринга, Т.СуммаОплаты ИЗ ТаблицаРегистраторовДоходов КАК Т) КАК ТаблицаРегистраторовДоходов
		...
		Пока ВыборкаПлатежей.Следующий() Цикл
			...
			Пока Выборка.Следующий() И СуммаОплаты > 0 Цикл
				...
				Для каждого ЭлементКорректировки Из СтруктураКорректировки Цикл
					...
					Если СуммаДокумента > 0 И СуммаПлатежа > 0 ИЛИ ЗначениеКорректировки <> 0 Тогда
						ДанныеПродажи["Сумма"+ИмяКолонки] = ДанныеПродажи["Сумма"+ИмяКолонки] - СуммаДокумента;
					КонецЕсли;
					
					Если СуммаДокумента > 0 И СуммаПлатежа > 0 Тогда
						ДанныеПлатежа["Доход"+ИмяКолонки] = ДанныеПлатежа["Доход"+ИмяКолонки] - СуммаПлатежа;
						Если ЗначениеКорректировки = 0 Тогда
							...
							ДанныеПлатежа.ДоходВсего = ДанныеПлатежа.ДоходВсего - СуммаПлатежа;
							СуммаОплаты = СуммаОплаты - СуммаПлатежа;
						КонецЕсли;
					КонецЕсли;
...
Функция ТекстЗапросаПоОплатамЗарплатыИНалогов()
	...
	|	УплатыНалогов.Регистратор КАК Регистратор,
	...
	|	И УплатыНалогов.Налог В (&ТипыНДФЛВзносы)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор,
	|	ВидРасходов,
