...
Процедура ВыгрузитьУстановленныеЦены()  
	...
	Если ИспользуетсяЦенообразование25 Тогда
		ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1000 РАЗЛИЧНЫЕ 
		               ...
		               |	И ЦеныНоменклатуры25СрезПоследних.Цена <> СтатусыПубликацииТоваровЯндексМаркет.ЦенаПродажи"; 
	...
	Иначе 
		ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1000 РАЗЛИЧНЫЕ 
					   ...
					   |	И ЦеныНоменклатурыСрезПоследних.Цена <> СтатусыПубликацииТоваровЯндексМаркет.ЦенаПродажи"; 
...
Функция ПолучитьРазницуОстатковТовараДляВыгрузки(МенеджерВТ) 
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 2000
       ...
       |	СУММА(ОстаткиНаСкладе.Свободно),
	   ...
	   |			И НЕ ОстаткиНаСкладе.Номенклатура ЕСТЬ NULL
	   |				ЛЕВОЕ СОЕДИНЕНИЕ  РегистрСведений.ОстаткиТоваровМаркетплейсов КАК ОстаткиТоваровМаркетплейсов
	   |				ПО  ОстаткиНаСкладе.Номенклатура = ОстаткиТоваровМаркетплейсов.Номенклатура
       |					И ОстаткиНаСкладе.Характеристика = ОстаткиТоваровМаркетплейсов.Характеристика
       |					И ОстаткиНаСкладе.Склад = ОстаткиТоваровМаркетплейсов.Склад
  	   |					
       |ГДЕ
       ...
       |	ОстаткиНаСкладе.Характеристика,
       |	ОстаткиНаСкладе.Склад
	   |ИМЕЮЩИЕ СУММА(ОстаткиНаСкладе.Свободно)<>СУММА(ЕСТЬNULL(ОстаткиТоваровМаркетплейсов.Количество,0))
	   |;
	   |////////////////////////////////////////////////////////////////////////////////
	   |УНИЧТОЖИТЬ ВтТовары";  
