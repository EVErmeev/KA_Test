...
Функция КНДПоИмениФайлаФНС(ИмяФайла) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(ИмяФайла, "ON_CHETOP") Тогда
		КНД = "1110379";
	Иначе
		Менеджер = НайтиМенеджерФормата(, ИмяФайла);
		Если Менеджер <> Неопределено Тогда
			...
			Исключение
				...
				Шаблон = НСтр(
					"ru = 'Менеджер %1, не удалось определить КНД, по причине:
					|%2'");
...
Функция КНДПоСвойствамДокумента(СвойстваДокумента) Экспорт
	...
	Иначе
		Менеджер = НайтиМенеджерФормата(ТипДокумента, ИмяФайла);
		Если Менеджер <> Неопределено Тогда
			Попытка
				КНД = Менеджер.КНД();
			Исключение
...
// Найти обрботку - менеджер формата электронного документа.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  - Неопределено - если не удалось найти менеджер по параметрам
...
//
Функция НайтиМенеджерФормата(ТипДокумента, ИмяФайлаОбмена) Экспорт
	Попытка
		Для Каждого Менеджер Из МенеджерыФорматов() Цикл
			МенеджерНайден = Ложь;
			Если ЗначениеЗаполнено(ТипДокумента) Тогда
				ПоддерживаемыеТипы = Менеджер.ПоддерживаемыеТипыДокументов();
				МенеджерНайден = ЗначениеЗаполнено(ПоддерживаемыеТипы.Найти(ТипДокумента));
			КонецЕсли;
			Если ЗначениеЗаполнено(ИмяФайлаОбмена) Тогда
				МенеджерНайден = Менеджер.ЭтоФайлФормата(ИмяФайлаОбмена);
			КонецЕсли;
			Если МенеджерНайден Тогда
				Возврат Менеджер;
			КонецЕсли;
		КонецЦикла;
	Исключение
		ПодсистемаРегистрацииОшибок = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
		Шаблон = НСтр(
			"ru = 'Менеджер %1, не удалось выполнить поиск менеджера формата ЭД, по причине:
			|%2'");
		ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Менеджер), ОписаниеОшибки());
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
...
// 
// Возвращаемое значение:
//  Массив Из см. НайтиМенеджерФормата
//
Функция МенеджерыФорматов() Экспорт
	СписокМетаданных = МетаданныеМенеджеровФорматов();
	Количество = СписокМетаданных.Количество();
	Если Количество > 0 Тогда
		Менеджеры = Новый Массив(Количество);
		Для Индекс = 0 По Менеджеры.ВГраница() Цикл
			ПолноеИмя = СписокМетаданных[Индекс].ПолноеИмя();
			Менеджеры[Индекс] = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
		КонецЦикла;
		Возврат Менеджеры;
	Иначе
...
Функция ЭтоОбработкаФормата(Объект) Экспорт
	Если Объект <> Неопределено Тогда
		...
		ОбработкиФорматов = МетаданныеМенеджеровФорматов();
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	...
	Иначе
		Попытка
			Менеджер = НайтиМенеджерФормата(ТипДокумента, Формат);
			Если ТипЗнч(Менеджер) <> Тип("Неопределено") И Менеджер.ЗаполнениеДанныхПодписантаДоступно() Тогда
				ЛокальноеИмя = "Файл";
				ДанныеПодписанта = НовыеДанныеПодписанта(Организация, СертификатПодписи, ВыбраннаяМЧД);
				Менеджер.ЗаполнитьДанныеПодписанта(ДанныеФайлаЭД, ДанныеПодписанта, Ошибки);
			КонецЕсли;
		Исключение
			ПодсистемаРегистрацииОшибок = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
			Шаблон = НСтр(
				"ru = 'Менеджер %1, не удалось заполнить данные подписанта, по причине:
				|%2'");
			ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Менеджер), ОписаниеОшибки());
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
				УровеньЖурналаРегистрации.Ошибка);
...
Функция ЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Экспорт
	...
	Менеджер = НайтиМенеджерФормата(ТипДокумента, Формат);
	Если ТипЗнч(Менеджер) = Тип("Неопределено") Тогда
	...
	Иначе
		Попытка
			Возврат Менеджер.ЗаполнениеДанныхПодписантаДоступно();
		Исключение
			ПодсистемаРегистрацииОшибок = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
			Шаблон = НСтр(
				"ru = 'Менеджер %1, не удалось определить доступность заполнения данных подписанта, по причине:
				|%2'");
			ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Менеджер), ОписаниеОшибки());
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
				УровеньЖурналаРегистрации.Ошибка);
	...
	Попытка
		...
		Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
			...
			Иначе
				...
				Менеджер = НайтиМенеджерФормата(, ДанныеФайлаЭД.ИдФайл);
				Если Менеджер <> Неопределено Тогда
					ТаблицаВозврата = Менеджер.ИнформацияОТоваре(ДанныеФайлаЭД);
				КонецЕсли;
...
Функция РазделыДополнительныхПолейФорматаЭлектронногоДокумента(ТипЭлектронногоДокумента, Формат) Экспорт
	...
	Иначе
		Менеджер = НайтиМенеджерФормата(ТипЭлектронногоДокумента, Формат);
		Попытка
			Менеджер.ЗаполнитьРазделыДополнительныхПолей(РазделыДополнительныхПолей);
		Исключение
			ПодсистемаРегистрацииОшибок = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
			Шаблон = НСтр(
				"ru = 'Менеджер %1, не удалось определить разделы дополнительных полей, по причине:
				|%2'");
			ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Менеджер), ОписаниеОшибки());
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
				УровеньЖурналаРегистрации.Ошибка);
...
Функция ДанныеЭлектронногоДокументаОсновныхТипов(ПараметрыПолученияДанных)
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда //чтение файлов без namespace по форматам ФНС
		...
		Иначе
			...
			Если Менеджер <> Неопределено Тогда
				Попытка
					Менеджер.ПрочитатьФайлОбмена(ДанныеФайлаЭД, ДеревоРазбора, НовыйЭД, ОшибкаРазбора);
				Исключение
					ПодсистемаРегистрацииОшибок = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
					Шаблон = НСтр(
						"ru = 'Менеджер %1, не удалось прочитать файл обмена, по причине:
						|%2'");
					ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Менеджер), ОписаниеОшибки());
					ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
						УровеньЖурналаРегистрации.Ошибка);
...
Функция ДанныеФайлаЭД(ДанныеФайла, ОписаниеОшибки, ПространствоИменФНС = Неопределено, ИмяКорневогоУзла = "", МягкоеЧтение = Ложь)
	...
	Если ЗначениеЗаполнено(ИдФайл) И Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Если Не МягкоеЧтение Тогда
			...
			Иначе
				Менеджер = НайтиМенеджерФормата(, ИдФайл);
				Если Менеджер <> Неопределено Тогда
					Попытка
						URIПространстваИмен = Менеджер.ПространствоИмен();
					Исключение
						ПодсистемаРегистрацииОшибок = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
						Шаблон = НСтр(
	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Попытка
			...
			ИначеЕсли URIПространстваИмен = "ON_AKTSVEROTP_1_972_01_05_01_01"
				...
				// У корневого свойства xdto-пакетов акта сверки взаиморасчетов не указан тип,
...
Функция МетаданныеМенеджеровФорматов()
