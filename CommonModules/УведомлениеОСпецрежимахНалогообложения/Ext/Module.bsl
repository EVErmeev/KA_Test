...
Процедура ЗапретитьРедактированиеРеквизитовСданногоОтчета(Форма)
	...
	СтатусОтправкиВЖурнале = СохраненныйСтатусОтправкиРеглОтчета(Форма.Объект);
...
Функция СохраненныйСтатусОтправкиРеглОтчета(Объект)
	Запрос = Новый Запрос;
...
Процедура НапечатитьСтруктуруСлужебный(Данные, Области)
	Для Каждого КЗ Из Данные Цикл
		Если ТипЗнч(КЗ.Значение) = Тип("Строка") Тогда 
			ВывестиСтрокуНаПечать(КЗ.Значение, КЗ.Ключ, Области, "-");
		ИначеЕсли ТипЗнч(КЗ.Значение) = Тип("Дата") Тогда 
			ВывестиДатуНаПечать(КЗ.Значение, КЗ.Ключ, Области, "-");
		...
		ИначеЕсли КЗ.Значение = Неопределено Тогда 
			ВывестиСтрокуНаПечать("", КЗ.Ключ, Области, "-");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура НапечататьСтруктуру(ИмяОтчета, СтруктураДанныхСтраницы, НомСтр, ИмяМакета, ПечатнаяФорма, ИННКПП, ВыводитьМакет = Истина) Экспорт 
	...
	НомСтр = НомСтр + 1;
	НапечатитьСтруктуруСлужебный(ИННКПП, МакетПФ.Области);
	НапечатитьСтруктуруСлужебный(СтруктураДанныхСтраницы, МакетПФ.Области);
...
Процедура НапечататьСтраницуСМногострочнойЧастью(Объект, Листы, СтруктураПараметров, СтраницаДляПечати, МногострочнаяЧасть, ИмяМакета, НомСтр, ИННКПП)
	...
	Для Инд = 0 По КоличествоСтраниц - 1 Цикл 
		...
		КонецЕсли;
		НапечатитьСтруктуруСлужебный(ИННКПП, МакетПФ.Области);
		НапечатитьСтруктуруСлужебный(СтраницаДляПечати, МакетПФ.Области);
		...
		Для Каждого Кол Из ТаблицаМногострочнаяЧасть.Колонки Цикл 
			...
			КонецЦикла;
		КонецЦикла;
		НапечатитьСтруктуруСлужебный(СтруктураПечатиДопСтрок, МакетПФ.Области);
