...
Процедура УстановитьСвойстваЭлементовНастройкиСчетовУчета(Форма, ЗаполнитьПоУмолчанию = Ложь) Экспорт
	...
	Для каждого ИмяСчета Из Настройки.СчетаСДинамическимиПараметрамиВыбора Цикл
		Если ТекущиеНастройкиРаздела.ЭлементыНастройки.НайтиПоЗначению(ИмяСчета) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ТекущиеНастройкиРаздела.ДолгосрочныеСчета.Найти(ИмяСчета) <> Неопределено И Не Настройки.РедактированиеВСписке Тогда
	...
	Если Настройки.ИспользуютсяДолгосрочныеСчетаУчета Тогда
		Если ТекущиеНастройкиРаздела.ДолгосрочныеСчета.Количество() Тогда
