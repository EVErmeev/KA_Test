...
//  Отказ - Булево -
//  ИзвлечьНоменклатуруИзОбъектаВКолонке - Строка - Имя колонки таблицы, из которой можно извлечь номенклатуру (если не передана явно).
// 
// Возвращаемое значение:
//  Структура:
...
Функция НайтиИлиСоздатьСоставныеПрослеживаемыеНомераГТД(Товары,
	...
	Отказ = Ложь,
	ИзвлечьНоменклатуруИзОбъектаВКолонке = "") Экспорт
	
	Если Не ПустаяСтрока(ИзвлечьНоменклатуруИзОбъектаВКолонке) Тогда
		
		ИзвлечьПолеИзКолонкиТаблицы(Товары, ИзвлечьНоменклатуруИзОбъектаВКолонке, "Номенклатура", "Номенклатура",
	...
	Если СоставнойНомерГТД = Неопределено
		И Не Отказ Тогда
		СоставнойНомерГТД = СоздатьСоставнойПрослеживаемыйНомерГТД(КомплектующиеДляСозданияСоставногоНомераГТД, Истина);
	КонецЕсли;
...
//  * КоличествоПоРНПТ - Число -
//  * Количество - Число -  
Функция РазобратьСоставныеПрослеживаемыеНомераГТД(ТоварыКСписанию, ТоварыКОприходованию) Экспорт
...
Функция СоздатьСоставнойПрослеживаемыйНомерГТД(КомплектующиеДляСозданияСоставногоНомераГТД, ЭтоОсновнойКомплект)
