...
Функция ОбъектМетаИнформации()
	...
	Результат.Вставить("Программа", Неопределено); // Creator
...
Функция ПолучитьЗначениеТипаHEXСтрока(ОбъектПарсинга)
	...
	Если ЗначениеЗаполнено(ПредставлениеСтроки) Тогда
...
Функция ИзвлечьИнформациюОДокументе(ОбъектДокумента)
	...
	Если Информация <> Неопределено Тогда
		ОбъектДокумента.МетаИнформация.ДатаСоздания = ПолучитьЗначение(Информация, "CreationDate");
		ОбъектДокумента.МетаИнформация.ДатаИзменения = ПолучитьЗначение(Информация, "ModDate");
		Если ЗначениеЗаполнено(ПолучитьЗначение(Информация, "Producer")) Тогда
			ОбъектДокумента.МетаИнформация.Программа = ПолучитьЗначение(Информация, "Producer", "");
		Иначе
			ОбъектДокумента.МетаИнформация.Программа = ПолучитьЗначение(Информация, "Creator", "");
		КонецЕсли;
		ОбъектДокумента.МетаИнформация.Программа = ПроверитьТекстНаСимволы(ОбъектДокумента.МетаИнформация.Программа);
...
//  Произвольное
//
Функция ПолучитьЗначение(ТекущийОбъект, ИмяЗначения, ЗначениеПоУмолчанию = Неопределено)
	...
	Попытка
		...
		Если ОбъектДокумента.Инициализирован Тогда
			...
			Если ТочкаНачала <> Неопределено Тогда
				ТочкаНачала = ПолучитьЗначение(ТочкаНачала, "Pages");
			КонецЕсли;
			
			Если ТочкаНачала <> Неопределено Тогда
				ВыборкаДереваСтраниц(ОбъектДокумента, ТочкаНачала);
			КонецЕсли;
			
			ОтразитьСостояниеЗамера(ОбъектДокумента, "Дерево страниц", ТекущийОтсчет);
			
			Если ОбъектДокумента.РежимПарсинга = "Текст" Тогда
...
//	Структура
//
Функция ПолучитьДанныеТребованияИзФайлаPDF(СодержаниеДокумента, Требование, ПараметрыОбъекта = Неопределено) Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("ДнейДоОтвета",  0);
	
	ДанныеДокумента = ПодготовитьСводкуДокументаPDF(СодержаниеДокумента, ПараметрыОбъекта);
