...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	|		И НЕ ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.ОтчетПереработчика2_5)
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(50, НСтр("ru='Неправильный тип записи в движениях выпуска продукции отчетом переработчика.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(65, НСтр("ru='Не заполнена операция учета себестоимости в движениях себестоимости товаров.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//-- НЕ УТ
	...
	|	""Себестоимость товаров"" КАК ИмяРегистра,
