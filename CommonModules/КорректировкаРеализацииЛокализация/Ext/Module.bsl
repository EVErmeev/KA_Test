...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	|	ДанныеДокументов.ТипКорректировки
	|
	|ИМЕЮЩИЕ
	...
	|		ИЛИ ДанныеДокументов.ТипКорректировки В(
	|				ЗНАЧЕНИЕ(Перечисление.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыКорректировки.КорректировкаДоВводаОстатковУслугИПрочихАктивов))
	|		)
	|
...
#Область УменьшениеНДС // (Дт 90.03 :: Кт 68.02 / 19.09)
	...
	|	0 КАК КоличествоДт,
	|	ВЫБОР
	|		КОГДА НЕ Строки.НаДоходыРасходы ИЛИ Операция.СтатьяРасходов.ПринятиеКНалоговомуУчету
	|		ТОГДА СУММА((ЕСТЬNULL(СуммыДокументовВВалютахУчета.СуммаНДСРегл, -Строки.СуммаНДС)))
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаНУДт,
	|	ВЫБОР
	|		КОГДА Строки.НаДоходыРасходы И НЕ Операция.СтатьяРасходов.ПринятиеКНалоговомуУчету
	|		ТОГДА СУММА((ЕСТЬNULL(СуммыДокументовВВалютахУчета.СуммаНДСРегл, -Строки.СуммаНДС)))
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
