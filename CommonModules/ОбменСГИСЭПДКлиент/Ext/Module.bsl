...
Процедура ИсходящиеДокументыФормироватьПриИзменении(ТекущиеДанные) Экспорт
	...
	Если РеквизитыВида.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.ЭТрН")
		...
		Или РеквизитыВида.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.ЭСВ") Тогда
	...
	Если ТипЗнч(Форма.ПереходыПоОшибкам) = Тип("ФиксированныйМассив")
		...
		Иначе
			...
			ИначеЕсли СтрНачинаетсяС(ТекущийПереход, "ХранимыеДанные") Тогда
				...
				ОткрытьФорму(ИмяФормыХранимыхДанных, ПараметрыФормы);		
...
Функция ПолучитьОтборХранимыхДанных(КонтейнерРеквизитов, Форма, ГруппаДанных) Экспорт
	...
	Если ТипЗнч(КонтейнерРеквизитов) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Контрагент = КонтейнерРеквизитов.Контрагент;
	...
	Результат.Вставить("ГруппаДанных", ГруппаДанных);	
...
Процедура ВывестиФормуВводаДанных(Форма, Знач ИмяКоманды, ТекущиеДанные = Неопределено, ЭтоВыбор = Истина, ЕстьХранимыеДанные = Ложь) Экспорт
	
	Если ИмяКоманды = "ЗаполнитьТитулПеревозчикаВыгрузкаПогрузкаКонтейнера" Тогда
		Если Форма.Объект.ВидОперации = 0 Тогда
			ИмяКоманды = "ТитулПеревозчикаВыгрузкаКонтейнера";
		ИначеЕсли (Форма.Объект.ВидОперации = 1 И ЗначениеЗаполнено(Форма.Объект.ТекущийПолученныйТитул)) Тогда
	...
	ОписаниеРеквизитовФормыРодителя = ОбменСГИСЭПДКлиентПовтИсп.РеквизитыПодчиненнойФормы(Форма.ИмяФормы);
	...
	Если Отказ = Ложь Тогда
		ОписаниеРеквизитовФормы = ОбменСГИСЭПДКлиентПовтИсп.РеквизитыПодчиненнойФормы(ИмяОбъекта + ".Форма." + ИмяФормыДанных);
		...
		ОткрытьФорму(ИмяОбъекта + ".Форма." + ИмяФормыДанных, ПараметрыОткрытияФормы,,,,,ОписаниеОповещения);
	КонецЕсли;
	...
	Для Каждого ТаблицаКиЗ Из ТаблицыФормы Цикл  
		...
		Иначе
			...
			Для ИндексСтроки = 1 По ВсегоСтрок Цикл
				...
				Если ЗначениеЗаполнено(ИдентификаторСтрокиРодителя) = Ложь 
					Или ТекущаяСтрока["ИдентификаторСтрокиРодителя"] = ИдентификаторСтрокиРодителя 
...
Процедура ВводАдреса(ИмяЭлемента, Форма, ВладелецАдреса = Неопределено) Экспорт
	
	СтруктураРеквизитовАдреса = ОбменСГИСЭПДКлиентСервер.ПолучитьСтруктуруАдресаУчастника(ИмяЭлемента, Форма);
...
Процедура ВводАдреса_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда	
		ОбменСГИСЭПДКлиентСервер.ЗаполнитьРеквизитыАдреса(ДополнительныеПараметры.Форма, Результат, ДополнительныеПараметры.ИмяЭлемента);
