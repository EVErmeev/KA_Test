...
Функция СоздатьКонфигурациюМодели(ВидПлана) Экспорт
	...
	ТелоЗапроса.Вставить("category_processing", ?(НастройкиВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланПродажПоКатегориям, "group", "no"));
...
Функция ОбработатьОтветСервисаВнутренний(СтруктураОтвета, ИмяДействия, ДополнительныйПараметр = Неопределено)
	...
	ИначеЕсли ИмяДействия = "ПолучениеПрогнозаСервиса" Тогда
		...
		ТипПлана = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДополнительныйПараметр.ВидПлана, "ТипПлана");
		ПланПродажПоКатегориям = (ТипПлана = Перечисления.ТипыПланов.ПланПродажПоКатегориям);
		
		Если ПланПродажПоКатегориям Тогда
