...
Процедура ЗаполнитьТаблицуИзКроссТаблицы(ЭтотОбъектТаблица, ОбъектТаблица, КроссТаблица, Параметры, СтруктураДействий = Неопределено, 
	...
	Для каждого СтрокаКроссТаблицы Из КроссТаблица Цикл
		...
		Для каждого СтрокаПериод Из ТаблицаПериоды Цикл
			...
			Если СтруктураРеквизитов.Свойство("Количество_") И НоваяСтрокаОбъектТаблица[СтруктураРеквизитов["Количество_"]] = 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДополнительныеПараметры.ТипПлана = "ПланЗакупок"
			...
			Если НоваяСтрока.Сумма = 0 И НоваяСтрока.Количество = 0 Тогда
				НоваяСтрока.Сумма = НоваяСтрока.Цена;
			ИначеЕсли НоваяСтрока.Сумма = 0 И НоваяСтрока.Количество <> 0 Тогда
