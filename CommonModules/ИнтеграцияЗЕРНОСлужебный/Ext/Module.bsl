...
Функция ОбъектXDTOВXML(ОбъектXDTO, ПространствоИмен, ИмяТипа, ПараметрыОбмена = Неопределено) Экспорт
	...
	Если ПараметрыОптимизации.ВерсияAPI <> ПакетыXDTO.ВерсияAPI Тогда
		Для Каждого ПакетXDTO Из ПакетыXDTO.Список Цикл
			ТекстXML = СтрЗаменить(
				ТекстXML,
				ПакетXDTO.ПространствоИмен,
				СтрЗаменить(ПакетXDTO.ПространствоИмен, ПакетыXDTO.ВерсияAPI, ПараметрыОптимизации.ВерсияAPI));
		КонецЦикла;
	КонецЕсли;
...
Функция ОбъектXDTOВСтруктуру(ОбъектXDTO, ПараметрыПреобразования = Неопределено, КорневыеУзлы = Неопределено, ЭтоВложенныйКорневойУзел = Ложь) Экспорт
	...
	Если ПроверятьКорневыеУзлы Тогда
		...
		Если ПараметрыКорневогоТипа <> Неопределено Тогда
			...
			ДанныеУзла.Вставить("Параметры", ПараметрыКорневогоТипа);
			КорневыеУзлы.Добавить(ДанныеУзла);
		ИначеЕсли Строка(ТипОбъектаXDTO) = "{http://schemas.xmlsoap.org/soap/envelope/}Fault" Тогда
			РеквизитыТипа = Новый Структура;
			РеквизитыТипа.Вставить("ИмяПоляРезультат", "ДанныеОшибки");
			ДанныеУзла = Новый Структура();
			ДанныеУзла.Вставить("Значение",  ОбъектXDTO);
			ДанныеУзла.Вставить("Параметры", РеквизитыТипа);
			КорневыеУзлы.Добавить(ДанныеУзла);
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыПреобразования() Экспорт
	...
	ПространстваИмен.Добавить(ПространствоИменПоИмениПакетаXDTO("zerno_ws_faults"));
	...
	ПараметрыПреобразования.КорневыеТипы.Вставить(
		ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_ws_types"), "ResponseCodeType"),
		РеквизитыТипа);
		
	РеквизитыТипа = РеквизитыКорневогоТипа("ИдентификаторЗаявки");
	РеквизитыТипа.ИмяПоля = "MessageID";
	ПараметрыПреобразования.КорневыеТипы.Вставить(
		ФабрикаXDTO.Тип(ПространствоИменПоИмениПакетаXDTO("zerno_ws_types"), "UUID"),
		РеквизитыТипа);
		...
		Если ТипЗнч(ЗначениеСвойства) = Тип("ОбъектXDTO") Тогда
			...
			Если ЗаполнятьСвойство Тогда
				...
				Если ТипЗнч(ЗначениеСвойстваПриемника) = Тип("ОбъектXDTO") Тогда
					ПреобразоватьПроизвольныйОбъектXDTOВОбъектXDTO(ЗначениеСвойства, ЗначениеСвойстваПриемника, ПараметрыПреобразования);
					...
					Если (СвойствоПриемника = Неопределено И СвойствоИсточника.Тип.Открытый)
						...
						Если ТипЗнч(ЭлементаСпискаПриемника) = Тип("ОбъектXDTO") Тогда
							...
							Если Не ОпасныйРежим Тогда
								...
								РеквизитыТипаСодержание = ПараметрыПреобразования
									.ТипыИмеющиеСодержаниеТега[ЭлементаСпискаПриемника.Тип()];
								Если РеквизитыТипаСодержание <> Неопределено
									И ЭлементСпискаИсточника.Последовательность().Количество() Тогда
									ЗначениеСодержания = ЭлементСпискаИсточника.Последовательность().ПолучитьТекст(0);
									ЭлементаСпискаПриемника[РеквизитыТипаСодержание.ИмяПоляПриемника] = ЗначениеСодержания;
								КонецЕсли;
...
Функция ПрефиксПространстваИмен(ПространствоИмен) Экспорт
	...
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/lots/1.0.3") Тогда
		Возврат "lots";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/common/1.0.3") Тогда
		Возврат "common";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/dictionary/1.0.3") Тогда
		Возврат "dict";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/organizations/1.0.3") Тогда
		Возврат "org";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/sdiz/1.0.3") Тогда
		Возврат "sdiz";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/gpb/1.0.3") Тогда
		Возврат "gpb";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/gpb-sdiz/1.0.3") Тогда
		Возврат "gpbsdiz";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/grain-monitor/1.0.3") Тогда
		Возврат "gm";
		
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/common-types/1.0.3") Тогда
		Возврат "common2";
		
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/faults/1.0.3") Тогда
		Возврат "fault";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/types/1.0.3") Тогда
		Возврат "wstypes";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/sdiz/1.0.3") Тогда
		Возврат "wssdiz";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/lots/1.0.3") Тогда
		Возврат "wslots";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/dictionaries/1.0.3") Тогда
		Возврат "wsdict";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/gpb/1.0.3") Тогда
		Возврат "wsgpb";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/gpb-sdiz/1.0.3") Тогда
		Возврат "wsgpbsdiz";
		
	ИначеЕсли ВРег(ПространствоИмен) = ВРег("urn://x-artefacts-mcx-gov-ru/fgiz-zerno/api/ws/grain-monitor/1.0.3") Тогда
		Возврат "wsgm";
...
Функция ОрганизацияКонтрагентXDTO(Реквизиты, СообщениеXML) Экспорт
	...
	Если Реквизиты.ТипОрганизации = Перечисления.ТипыОрганизацийЗЕРНО.ЮридическоеЛицо Тогда
		...
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO,        "Name",        Реквизиты.Наименование, СообщениеXML);
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO,        "INN",         Реквизиты.ИНН,          СообщениеXML);
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO,        "OGRN",        Реквизиты.ОГРН,         СообщениеXML);
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO,        "KPP",         Реквизиты.КПП,          СообщениеXML);
	...
	ИначеЕсли Реквизиты.ТипОрганизации = Перечисления.ТипыОрганизацийЗЕРНО.ИндивидульныйПредприниматель Тогда
		...
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "FamilyName", Реквизиты.Фамилия, СообщениеXML);
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "FirstName", Реквизиты.Имя, СообщениеXML);
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "INN", Реквизиты.ИНН, СообщениеXML);
		ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "OGRN", Реквизиты.ОГРН, СообщениеXML);
	...
	ИначеЕсли Реквизиты.ТипОрганизации = Перечисления.ТипыОрганизацийЗЕРНО.ИностраннаяОрганизация
		...
		Если ЗначениеЗаполнено(Реквизиты.ИНН) Тогда
			...
			Если ЗначениеЗаполнено(Реквизиты.КодРАФП) Тогда
				ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "RAFP", Реквизиты.КодРАФП, СообщениеXML);
			КонецЕсли;
			СтранаРегистрацииXDTO = ИнтеграцияИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, "Country", ТипОрганизацииXDTO);
			ЗаполнитьСвойствоXDTO(СтранаРегистрацииXDTO, "alpha_3", Реквизиты.КодАльфа3, СообщениеXML);
			ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "Country", СтранаРегистрацииXDTO, СообщениеXML);
			
			АдресРФXDTO = ИнтеграцияИС.ОбъектXDTO(ПространствоИмен, "AddressRFType");
			ЗаполнитьСвойствоXDTO(АдресРФXDTO, "PostalCode", Реквизиты.ЮридическийАдресИндекс, СообщениеXML);
			ЗаполнитьСвойствоXDTO(АдресРФXDTO, "Address", Реквизиты.ЮридическийАдрес, СообщениеXML);
			ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "Address", АдресРФXDTO, СообщениеXML);
		...
		Иначе // ИностраннаяОрганизацияБезРегистрацииВРФ
			...
			ЗаполнитьСвойствоXDTO(АдресXDTO, "Address", Реквизиты.ЮридическийАдресИндекс, СообщениеXML);
			ЗаполнитьСвойствоXDTO(ТипОрганизацииXDTO, "Address", АдресXDTO, СообщениеXML);
...
Функция КонвертСообщенияXML(СообщениеXML, ПараметрыСообщения, ИдентификаторЗаявки, ДополнительныеПараметры = Неопределено)
	...
	ТелоКонверта = СтрЗаменить(ТелоКонверта, "%ВерсияAPI%",                        ПараметрыОптимизации.ВерсияAPI);
