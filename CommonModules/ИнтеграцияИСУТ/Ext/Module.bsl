...
//
Процедура ПроверитьЗаполнениеАкцизныхМарокПоСсылкам(Источники, Отказ, НаДату = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияУТ.ЕстьРеквизитТабЧастиОбъекта(ПолеТаблицыАкцизныеМарки, МетаданныеИсточников, ИмяТаблицыАкцизныеМарки) Тогда
		ИмяТаблицыАкцизныеМарки = "ШтрихкодыУпаковок";
		ПолеТаблицыАкцизныеМарки = "ШтрихкодУпаковки";
	КонецЕсли;
	
	Реквизиты = СтрРазделить(Менеджер.ИменаРеквизитовДляЗаполненияПараметровУказанияСерий(),",",Ложь);
	...
	Для Каждого СтрокаИсточкик Из ИсточникСоответствие Цикл
		...
		ПроверитьЗаполнениеАкцизныхМарок(СтрокаИсточкик.Значение, Отказ, НаДату);
	КонецЦикла;
...
Процедура ПроверитьЗаполнениеАкцизныхМарок(Объект, Отказ, НаДату = Неопределено, РеквизитыОбъекта = Неопределено) Экспорт
	...
	Иначе
		Если ТипЗнч(Объект[ИмяТаблицыАкцизныеМарки]) = Тип("ТаблицаЗначений") Тогда
			ШтрихкодыУпаковок = Объект[ИмяТаблицыАкцизныеМарки].ВыгрузитьКолонку(ПолеТаблицыАкцизныеМарки);
		Иначе 
