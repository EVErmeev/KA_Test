...
Процедура РеквизитыОрганизацийКонтрагентов(ТаблицаОрганизацияКонтрагентПодразделение, ЗаполнитьКодыРАФП = Истина) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОрганизацияКонтрагентПодразделение Цикл
		...
		Если ТипОрганизации = Перечисления.ТипыОрганизацийЗЕРНО.ИностраннаяОрганизация
			...
			СоответствиеСтрок.Вставить(СтрокаТаблицы, Истина);
			...
			Попытка
				...
				Если СведенияЮрЛицаПоДаннымСервиса <> Неопределено Тогда
					Если ЗначениеЗаполнено(СведенияЮрЛицаПоДаннымСервиса.ОписаниеОшибки) Тогда
						...
						КонецЕсли;
					Иначе
						КлючИЗначение.Ключ.КодРАФП = СведенияЮрЛицаПоДаннымСервиса.КодРАФП;
...
Функция СписокАвтоподбораПоОписаниюТипов(ОписаниеТипов, ТекстДляАвтоПодбора,
	...
	Если СтрДлина(ТекстДляАвтоПодбора) >= МинимальнаяДлинаСтрокиПоиска Тогда
		...
		Запрос.Текст = ТекстЗапросаАдресаДляАвтоподбора(ОписаниеТипов);
...
Функция СписокАвтоподбораСкладКонтрагент(ТекстДляАвтоПодбора,
	ДобавлятьСлужебныеЭлементы = Истина, МинимальнаяДлинаСтрокиПоиска = 1) Экспорт
	
	Возврат СписокАвтоподбораПоОписаниюТипов(
		 Метаданные.ОпределяемыеТипы.СкладКонтрагентЗЕРНО.Тип,
		 ТекстДляАвтоПодбора,
...
Функция СписокАвтоподбораЗемельныйУчасток(ТекстДляАвтоПодбора,
	ДобавлятьСлужебныеЭлементы = Истина, МинимальнаяДлинаСтрокиПоиска = 1) Экспорт
	...
	Если СтрДлина(ТекстДляАвтоПодбора) >= МинимальнаяДлинаСтрокиПоиска Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 10
		|	ЗемельныеУчасткиИС.Ссылка                   КАК Ссылка,
		|	ЗемельныеУчасткиИС.Адрес                    КАК Адрес,
		|	ЗемельныеУчасткиИС.АдресСтрокой             КАК АдресСтрокой,
		|	ЗемельныеУчасткиИС.ПлощадьЗемельногоУчастка КАК ПлощадьЗемельногоУчастка
		|ИЗ
		|	Справочник.ЗемельныеУчасткиИС КАК ЗемельныеУчасткиИС
		|ГДЕ
		|	ЗемельныеУчасткиИС.АдресСтрокой ПОДОБНО &СтрокаПоиска
		|
		|УПОРЯДОЧИТЬ ПО
		|	АдресСтрокой";
		Выборка = Запрос.Выполнить().Выбрать();
		...
		Пока Выборка.Следующий() Цикл
			...
			Данные = Новый Структура;
			Данные.Вставить("ВладелецАдреса",           Выборка.Ссылка);
			Данные.Вставить("Адрес",                    Выборка.Адрес);
			Данные.Вставить("ПредставлениеАдреса",      Выборка.АдресСтрокой);
			Данные.Вставить("КомментарийАдреса",        ИнтеграцияЗЕРНО.КомментарийКонтактнойИнформации(Данные.Адрес));
			Данные.Вставить("ПлощадьЗемельногоУчастка", Выборка.ПлощадьЗемельногоУчастка);
			
			МассивСтрок = Новый Массив;
			СобытияФормИС.ФорматироватьСтрокуДляСпискаВыбора(Выборка.АдресСтрокой, ПодстрокаПоиска, МассивСтрок);
	...
	Если ДобавлятьСлужебныеЭлементы Тогда
		ОписаниеМетаданных = СобытияФормИС.ОписаниеМетаданныхПоОписаниюТипов(
			Новый ОписаниеТипов("СправочникСсылка.ЗемельныеУчасткиИС"));
		Для Каждого КлючИЗначение Из ОписаниеМетаданных Цикл
			Данные = КлючИЗначение.Значение;
			Данные.Вставить("Действие", "ОткрытьФорму");
			Данные.Вставить("ИмяФормы", "ФормаВыбора");
