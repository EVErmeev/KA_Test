...
Функция ВыгрузитьЗаявлениеНаОтменуМЧД(НомерДоверенности, ПричинаОтмены) Экспорт
	...
	ИначеЕсли ДанныеДоверенности.ТипОрганизации = "ИП" Тогда
		...
		ЗаписьXML.ЗаписатьАтрибут("ОГРНИП", ДанныеДоверенности.ДоверительИП_ОГРНИП);
		ЗаписьXML.ЗаписатьКонецЭлемента();
...
Функция ОбщиеСвойстваДоверенностей(Доверенности) Экспорт
	...
	Запрос.Текст = 
		...
		|	МашиночитаемыеДоверенностиКонтрагентов.ПолномочияОграничены КАК Поле1
		|ИЗ
		|	Справочник.МашиночитаемыеДоверенностиКонтрагентов КАК МашиночитаемыеДоверенностиКонтрагентов
		|ГДЕ
...
Функция ПрочитатьАрхивСДоверенностьюИПодписью(ДанныеАрхива) Экспорт
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, КраткийТекстОшибки);
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		...
		Для Каждого Данные Из ДанныеПроверки Цикл
			...
			Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
				...
				Если ПротоколПроверкиJSON <> Неопределено Тогда
					...
					ПротоколПроверки = ПрочитатьПротоколПроверкиJSON(ПротоколПроверкиJSON);
					ПротоколПроверки.ПроверкаМЧД.ПроверкаПолномочий = Данные.ПроверкаПолномочий;
					...
					ЗаписьНабора.ПодписьВерна = ПротоколПроверки.ПроверкаПодписиДокумента.Успех
						И ДоверенностьПроверенаУспешно(ПротоколПроверки.ПроверкаМЧД);
...
Функция ТабличныйДокументМЧД(ДвоичныеДанные, ЭтоМЧДКонтрагента = Ложь) Экспорт
	...
	ОбластьПереченьПолномочий.Параметры.ПереченьПолномочий = ТекстПолномочий(СтруктураДанных);;
	ТабличныйДокумент.Вывести(ОбластьПереченьПолномочий);
	
	Если СтруктураДанных.Свойство("ВозможностьПередоверия") Тогда
		Если СтруктураДанных.ВозможностьПередоверия = "1" Тогда
			ТабличныйДокумент.Вывести(ОбластьОднократногоПередоверия);
		ИначеЕсли СтруктураДанных.ВозможностьПередоверия = "2" Тогда
			ТабличныйДокумент.Вывести(ОбластьБезПраваПередоверия);
		ИначеЕсли СтруктураДанных.ВозможностьПередоверия = "3" Тогда
...
//    ** Ключ - СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций
//    ** Значение - Массив из см. МашиночитаемыеДоверенности.НоваяОшибкаПроверкиПолномочийПриПодписании
//
...
Функция ДанныеИзФайлаОбмена(ВходящиеДанные) Экспорт
	...
	ИначеЕсли ВерсияФормата = ФорматМЧД_2022_Версия_002() Тогда
		Возврат ДанныеИзФайлаОбменаВУтвержденномФорматеВерсия002(ВходящиеДанные);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПравилоНастроено(Правило) Экспорт
	...
	Скрипт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Правило, "Скрипт");
...
Функция ПолномочияОграничены(ДанныеДоверенности = Неопределено, Доверенность = Неопределено) Экспорт
	...
	Если ДанныеДоверенности = Неопределено Тогда
		...
		ДанныеДоверенности = ДанныеИзФайлаОбмена(ДвоичныеДанныеДоверенности);
...
Функция ТекстПолномочий(ДанныеДоверенности = Неопределено, Доверенность = Неопределено) Экспорт
	...
	Результат = СтрСоединить(СтрокиПолномочий, Символы.ПС);
...
Функция ДанныеИзФайлаОбменаВУтвержденномФормате(ВходящиеДанные)
	...
	ИначеЕсли ДанныеДоверенности.ТипОрганизации = "ИП" Тогда
		...
		ДанныеДоверенности.Вставить("ДоверительИП_ОГРНИП", СведенияДоверителя.ОГРНИП);
...
//  Структура - См. НовыеДанныеМЧД
//
Функция ДанныеИзФайлаОбменаВУтвержденномФорматеВерсия002(ВходящиеДанные)
...
Процедура ЗаполнитьДовериеПоФормату002(ДанныеДоверенности, Документ)
	...
	ДанныеДоверенности.Вставить("НомерРодительскойДоверенности", "");		
	ДанныеДоверенности.Вставить("ВозможноПередоверие", ?(СведенияДоверенности.ПрПередов = "1", Истина, Ложь));
	ДанныеДоверенности.Вставить("ВозможностьПередоверия", СведенияДоверенности.ПрПередов);
	ДанныеДоверенности.Вставить("ФИО", Новый Массив);
	ДанныеДоверенности.Вставить("УдостоверенияЛичности", Новый Массив);	
...
Процедура ЗаполнитьПередовериеПоФормату002(ДанныеДоверенности, Документ)
	...
	ДанныеДоверенности.Вставить("НомерОсновнойДоверенности", СведенияОбОсновнойДоверенности.НомДовер0);
...
Функция ИННДоверителейВУтвержденномФормате_Версия_002(ДанныеДоверенности)
	...
	Доверенность = ДанныеИзФайлаОбменаВУтвержденномФорматеВерсия002(ДанныеДоверенности);
...
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	...
	Попытка
		ДанныеXDTO = ПрочитатьДанныеXML(ДанныеДоверенности);
