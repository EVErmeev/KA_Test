...
Процедура ИнициализироватьВтОстаткиДляРеклассификации(Запрос, ТекстыЗапроса)
	...
	ПорядокНачисленияАмортизацииУпр =  ПараметрыУчетнойПолитики.ПорядокНачисленияАмортизации;
	ПорядокНачисленияАмортизацииРегл = ПараметрыУчетнойПолитики.ПорядокНачисленияАмортизацииРегл;

	ЗапросОстаткиОбесценения.УстановитьПараметр("ПорядокНачисленияАмортизацииУпр", ПорядокНачисленияАмортизацииУпр);
	ЗапросОстаткиОбесценения.УстановитьПараметр("ПорядокНачисленияАмортизацииРегл",ПорядокНачисленияАмортизацииРегл);
	...
	|	ВЫБОР
	|		КОГДА &ПорядокНачисленияАмортизацииРегл = ЗНАЧЕНИЕ(Перечисление.ПорядокНачисленияАмортизации.СоСледующегоМесяца)
	|			ТОГДА ОбесценениеВНА.ОбесценениеРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ОбесценениеРегл,
	|	ВЫБОР
	|		КОГДА &ПорядокНачисленияАмортизацииУпр = ЗНАЧЕНИЕ(Перечисление.ПорядокНачисленияАмортизации.СоСледующегоМесяца)
