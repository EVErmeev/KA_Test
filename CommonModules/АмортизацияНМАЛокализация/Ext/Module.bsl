...
Процедура ДополнитьТекстЗапросаПараметрыРасчетаАмортизации(СписокЗапросов, ПараметрыРасчета) Экспорт
	...
	|
	|	ЕСТЬNULL(Изменения_БУ.ЛиквидационнаяСтоимость, ПараметрыАмортизацииНМАБУ.ЛиквидационнаяСтоимость) КАК ЛиквидационнаяСтоимость,
	|
	|	ЕСТЬNULL(ОбесценениеНаНачалоМесяца.ОстатокОбесцененияРегл, 0) КАК ОстатокОбесцененияРегл,
	|
	...
	|	ЕСТЬNULL(Изменения_БУ.КоэффициентБУ, ПараметрыАмортизацииНМАБУ.КоэффициентБУ) КАК КоэффициентБУ,
	|
	// РесурсНазначенный
	|	ВЫБОР 
	...
	|			ТОГДА ЕСТЬNULL(Изменения_БУ.ОбъемНаработки, ПорядокУчетаНМА.ОбъемНаработки)
	|		ИНАЧЕ ЕСТЬNULL(Изменения_БУ.СрокИспользования, ПараметрыАмортизацииНМАБУ.СрокИспользованияДляВычисленияАмортизации)
	|	КОНЕЦ КАК РесурсНазначенный,
	|
	// РесурсВыработанныйЗаТекущийПериод
	|	ВЫРАЗИТЬ(ВЫБОР 
...
Функция ТекстЗапросаСуммаАмортизацииБУ(ПараметрыТекстаЗапроса)
	...
	|		ТОГДА ВЫБОР 
	|			КОГДА СписокНМА.ЭтоДоначисление
	|				ТОГДА ВЫРАЗИТЬ((СписокНМА.СтоимостьДляВычисленияАмортизации - &СуммаДоначисления)
	|								* (ВЫРАЗИТЬ(СписокНМА.РесурсВыработанныйЗаТекущийПериод КАК ЧИСЛО(20,8)) 
	|								/ ВЫРАЗИТЬ(СписокНМА.РесурсНазначенный - СписокНМА.РесурсВыработанныйЗаПрошлыйПериод КАК ЧИСЛО(20,8))) КАК ЧИСЛО(31,2))
	|
	|			ИНАЧЕ ВЫРАЗИТЬ((СписокНМА.СтоимостьДляВычисленияАмортизации - &СуммаДоначисления - СписокНМА.ЛиквидационнаяСтоимость) 
	|								* (ВЫРАЗИТЬ(СписокНМА.РесурсВыработанныйЗаТекущийПериод КАК ЧИСЛО(20,8))
	|								/ ВЫРАЗИТЬ(СписокНМА.РесурсНазначенный - СписокНМА.РесурсВыработанныйЗаПрошлыйПериод КАК ЧИСЛО(20,8))) КАК ЧИСЛО(31,2))
	|		КОНЕЦ
	|
	...
	|						- СписокНМА.КоэффициентБУ * (СписокНМА.РесурсВыработанныйЗаПрошлыйПериод - СписокНМА.ОтработанныхМесяцевПолныхЛет) КАК ЧИСЛО(20,8)) КАК ЧИСЛО(31,2))
	|
		// Линейный
	|	КОГДА НЕ СписокНМА.АмортизацияНачисляетсяПоСтандартамМУ
	|			И СписокНМА.МетодНачисленияАмортизацииБУ = ЗНАЧЕНИЕ(Перечисление.СпособыНачисленияАмортизацииНМА.Линейный)
	|
	|		ТОГДА ВЫРАЗИТЬ((СписокНМА.ПервоначальнаяСтоимостьБУ - &ОстатокОбесценения) / СписокНМА.СрокПолезногоИспользованияБУ КАК ЧИСЛО(31,2))
	|   
