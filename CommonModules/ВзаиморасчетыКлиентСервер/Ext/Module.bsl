...
Процедура РассчитатьКонечноеСальдоДтКт(ИтоговаяЗапись, ДетальныеЗаписи, СверкаПоДоговорам = Ложь, ЭтоПоляКонтрагента = Ложь) Экспорт
	...
	Если ИтоговаяЗапись[ИмяПоля.НачальноеСальдоДт] <> 0
		...
		КонечноеСальдоДт = ИтоговаяЗапись[ИмяПоля.НачальноеСальдоДт] + ОборотДт - ОборотКт;
		КонечноеСальдоКт = ИтоговаяЗапись[ИмяПоля.НачальноеСальдоКт] - ОборотДт + ОборотКт;
		ИтоговаяЗапись[ИмяПоля.КонечноеСальдоДт] = Макс(КонечноеСальдоДт, 0) + Макс(-КонечноеСальдоКт, 0);
		ИтоговаяЗапись[ИмяПоля.КонечноеСальдоКт] = Макс(КонечноеСальдоКт, 0) + Макс(-КонечноеСальдоДт, 0);;
	КонецЕсли;
