...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	Строки.МестоХранения КАК МестоУчетаДт,
	|
	...
	|		ТОГДА Строки.ПодразделениеСкладскойТерритории
	|		КОГДА Строки.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеНаКомиссию),
	|									ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыПереданныеПартнерам),
	|									ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути),
	|									ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеПереработчику))
	...
	|		ТОГДА Строки.ПодразделениеДоговора
	...
	|									ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеПереработчику))
	|			ИЛИ Строки.МестоХранения ССЫЛКА Справочник.Партнеры
	|		ТОГДА ЕСТЬNULL(Строки.КонтрагентВидаЗапасов, Строки.Контрагент)
	|		КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|		ТОГДА Строки.ГруппаПродукции
	|		ИНАЧЕ Строки.МестоХранения
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|			ТОГДА ЕСТЬNULL(Строки.ВидЗапасов.ГруппаФинансовогоУчета,
	|				ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))
	|		ИНАЧЕ ЕСТЬNULL(Аналитика.Номенклатура.ГруппаФинансовогоУчета,
	|			ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))
	...
	|	Аналитика.СкладскаяТерритория.Подразделение КАК ПодразделениеСкладскойТерритории,
	|	Аналитика.Договор.Подразделение КАК ПодразделениеДоговора,
	|	Аналитика.Подразделение КАК ПодразделениеАналитики,
	|	ВЫБОР
	...
	|	Строки.ВидЗапасов.Контрагент КАК КонтрагентВидаЗапасов,
