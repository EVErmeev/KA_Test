...
Функция ВДокументеДругиеВидыПродукцииИНетЗерна(ИмяДокумента, ДопОтборТовары = "")
	...
	|			ИЛИ ДанныеПродукции.ОсобенностьУчета <> ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродуктыПереработкиЗерна)) КАК ДругиеОсобенностиУчета,
	|	МАКСИМУМ(ДанныеПродукции.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.Зерно)
	|			ИЛИ ДанныеПродукции.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродуктыПереработкиЗерна)) КАК ЕстьЗерно
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		ТаблицаТовары.Ссылка КАК Ссылка,
