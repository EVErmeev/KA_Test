...
Функция ДанныеСертификатаУчетнойЗаписиЭДО(ИдентификаторУчетнойЗаписи) Экспорт
	...
	Возврат МодульОбменСГИСЭПД.ДанныеСертификатаУчетнойЗаписиЭДО(ИдентификаторУчетнойЗаписи);
		...
		Если ЗначениеЗаполнено(СообщениеТекущегоТитула) Тогда
			...
			Если ЗначениеЗаполнено(Объект.ТекущийПолученныйТитул) Тогда
				...
				Если СообщениеТекущегоПолученногоТитула <> Неопределено Тогда
					...
					РеквизитыСообщениеТекущегоТекущегоТитула = ОбменСГИСЭПДВызовСервера.ЗначенияРеквизитовОбъекта(СообщениеТекущегоТитула, "Дата");
					ПоследнийТитулВходящий = РеквизитыСообщениеТекущегоПолученногоТитула.Дата > РеквизитыСообщениеТекущегоТекущегоТитула.Дата;
				Иначе
					ПоследнийТитулВходящий = Ложь;	
				КонецЕсли;		
			КонецЕсли;
			Если ПоследнийТитулВходящий = Ложь Тогда
				МассивСтрок = Новый Массив;
...
Процедура УстановитьДоступностьЭлементовЭТрН(Форма) Экспорт
	...
	// Документ еще не отправлялся - дадим редактировать
	Если ДанныеСостоянияЭДО.СуществуетАктуальныйДокумент = Ложь Или ТребуетсяПовторно = Истина Тогда 
...
Процедура УстановитьТекущийДоступныйТитулЭТрН(Объект, ТитулыПоДокументу = Неопределено, ВерсииТитулов = Неопределено, ВводимДопТитул = Ложь, ТребуетсяПовторно = Ложь) Экспорт
	...
	Если Объект.РольУчастника = 1 Тогда	
		...
		ИначеЕсли (ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8")) <> Неопределено
		 	Или ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул7")) <> Неопределено)
			И ТребуетсяПовторно Тогда
			МассивВерсийЗамена = ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8"));
			МассивВерсийПереадресовка = ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул7"));
			ДатаПоследнейЗамены = Дата(1, 1, 1);
			Если МассивВерсийЗамена <> Неопределено Тогда
				ДатаПоследнейЗамены = МассивВерсийЗамена[МассивВерсийЗамена.ВГраница()].Дата;
			КонецЕсли;
			ДатаПоследнейПереадресовки = Дата(1, 1, 1);
			Если МассивВерсийПереадресовка <> Неопределено Тогда
				ДатаПоследнейПереадресовки = МассивВерсийПереадресовка[МассивВерсийПереадресовка.ВГраница()].Дата;
			КонецЕсли;
			Если ДатаПоследнейЗамены > ДатаПоследнейПереадресовки Тогда
				ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8");	
			Иначе
				ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул7");	
			КонецЕсли;
	...
	ИначеЕсли (Объект.РольУчастника = 2 
		...
		Если (ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул2"))))
		...
		ИначеЕсли ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул7"))) 
	...
	Если ТекущийДоступныйТитул = Неопределено И Объект.РольУчастника = 3 Тогда	
		...
		ИначеЕсли (ТребуетсяПовторно Или ЗначениеЗаполнено(ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1"))) = Ложь) Тогда
	...
	Если (Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул7")
			Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8")) Тогда
	...
	ИначеЕсли (Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул7")
			Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул8")) Тогда
...
Процедура УстановитьДоступностьЭлементовЭЗН(Форма) Экспорт
	...
	УстановитьТекущийДоступныйТитулЭЗН(Объект);
	
	ЭтоНеЗаконченнаяОтправка = Ложь;
	УстановитьДоступностьЭлементовПоСостоянию(Форма, ДанныеСостоянияЭДО, ЭтоНеЗаконченнаяОтправка);
	
	Если ЭтоНеЗаконченнаяОтправка = Истина Тогда
		УстановитьТекущийДоступныйТитулЭЗН(Объект, , ЭтоНеЗаконченнаяОтправка);	
	КонецЕсли;
	
	// По имеющимся титулам
	Элементы.СтраницаСодЗНИнфФт.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ТитулФрахтователяИдентификаторФайла);	
	Элементы.СтраницаСодЗНИнфФщ.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ТитулФрахтовщикаИдентификаторФайла);
	Элементы.СтраницаСодЗНИнфФтПодача.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ТитулФрахтователяПодачаИдентификаторФайла);	
	Элементы.СтраницаСодЗНИнфФщВозврат.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ТитулФрахтовщикаВозвратИдентификаторФайла);
...
Процедура УстановитьТекущийДоступныйТитулЭЗН(Объект, ТитулыПоДокументу = Неопределено, ТребуетсяПовторно = Ложь) Экспорт
	...
	// Фрахтовщик
	Если Объект.РольУчастника = 1 Тогда	
		Если Объект.ТекущийПолученныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул3") 
			И ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул4")) = Неопределено Тогда
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул4");	
		ИначеЕсли Объект.ТекущийПолученныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул1")
			И ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул2")) = Неопределено Тогда
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул2");
		КонецЕсли;					
	// Фрахтователь
	ИначеЕсли Объект.РольУчастника = 3 Тогда	
		Если Объект.ТекущийПолученныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул2") 
			И ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул3")) = Неопределено Тогда	
			ТекущийДоступныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул3");
		ИначеЕсли 
			ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул1")) = Неопределено Тогда
...
Процедура ПосчитатьКоличествСтрокВсехТаблиц(Форма)
	...
	Если ГруппаТаблицы <> Неопределено И ГруппаТаблицы.ПодчиненныеЭлементы.Количество() > 1 Тогда
	...
	Для К = 1 По СтрДлина(ДатаСтрока) + 1 Цикл
		...
		Иначе
			...
			Если НачалисьСимволыВремени = Ложь Тогда 
				Если СтрДлина(ТекущаяПодстрока) = 2 И СтруктураПодстрок.День = Неопределено Тогда
...
Процедура УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма, ИмяЭлемента, Значение = Неопределено, Перезаписывать = Истина, КонтейнерОтображаемойВерсии = Неопределено) Экспорт
	...
	Если Перезаписывать = Истина Или ЗначениеЗаполнено(ТекущееЗначение) = Ложь Тогда
		Если Значение = Неопределено И ЗначениеЗаполнено(ТекущееЗначение) Тогда
			СтруктураВерсииТитула.Удалить(ИмяЭлемента);
		Иначе	
			СтруктураВерсииТитула.Вставить(ИмяЭлемента, Значение);
			Если ТипЗнч(Значение) = Тип("Дата") Тогда
				СтруктураВерсииТитула.Вставить(ИмяЭлемента + "ИспользуетсяUTC", Ложь);	
			КонецЕсли;
		КонецЕсли;
...
Функция СуммаПоСтрокамТаблицыИзСтруктурыФормы(Форма, ИмяТаблицы, ИмяКолонки, КонтейнерОтображаемойВерсии = Неопределено) Экспорт
	...
	Пока СтруктураВерсииТитула.Свойство(КлючКолонки, ЗначениеКолонки) Цикл
...
Процедура УдалитьРеквизитыПоГруппеДанных(Форма, ГруппаДанных, КонтейнерОтображаемойВерсии = Неопределено) Экспорт
	...
	Если ОписаниеГруппы.ЕстьХранимыеДанные = Истина Тогда
		Для Каждого ИмяРеквизита Из ОписаниеГруппы.Реквизиты Цикл
			УстановитьЗначениеРеквизитаВСтруктуреФормы(Форма, ИмяРеквизита, Неопределено, Истина, КонтейнерОтображаемойВерсии);	
		КонецЦикла;
	КонецЕсли;
...
Процедура ИзменитьОформлениеФормы(Форма, ИмяЭлемента = Неопределено) Экспорт
	...
	Если Форма.ИмяФормы = "Документ.ЭлектроннаяТранспортнаяНакладная.Форма.ОсновнаяФорма" Тогда
		...
		Элементы.ГруппаРеквизитаЗаполнитьТитулПрвЗаменыДокументПричинЗаменыРеквизитыСторон.Видимость = Форма.ВариантПричиныЗамены = 1;
		...
		Элементы.ГруппаРеквизитаЗаполнитьДопТитулПрвЗаменыДокументПричинЗаменыРеквизитыСторон.Видимость = Форма.ВариантПричиныЗаменыДоп = 1;
		...
		ИначеЕсли ИмяЭлемента <> "ВариантПриемки" Тогда
			Форма.ВариантПриемки = 0;
		КонецЕсли;
		Элементы.ГруппаРеквизитаТитулГрузополучателяПричинаОтказаВПриемеГруза.Видимость = Форма.ВариантПриемки = 1;	
		Для Каждого ТекущийЭлемент Из Элементы.ГруппаРеквизитыСодИнфГП.ПодчиненныеЭлементы Цикл
			Если ТекущийЭлемент.Имя <> "ГруппаВариантПриемки" И ТекущийЭлемент.Имя <> "ГруппаРеквизитаТитулГрузополучателяПричинаОтказаВПриемеГруза" Тогда
				ТекущийЭлемент.Видимость = Форма.ВариантПриемки = 0;	
			КонецЕсли;
		КонецЦикла;
		Элементы.ГруппаТаблицыСодИнфГП.Видимость = Форма.ВариантПриемки > 0;
		...
		ИначеЕсли ИмяЭлемента <> "ВариантПриемкиДоп" Тогда
			Форма.ВариантПриемкиДоп = 0;
		КонецЕсли;
		Элементы.ГруппаРеквизитаДопТитулГрузополучателяПричинаОтказаВПриемеГруза.Видимость = Форма.ВариантПриемкиДоп = 1;	
		Для Каждого ТекущийЭлемент Из Элементы.ГруппаРеквизитыДопСодИнфГП.ПодчиненныеЭлементы Цикл
			Если ТекущийЭлемент.Имя <> "ГруппаВариантПриемкиДоп" И ТекущийЭлемент.Имя <> "ГруппаРеквизитаДопТитулГрузополучателяПричинаОтказаВПриемеГруза" Тогда
				ТекущийЭлемент.Видимость = Форма.ВариантПриемкиДоп = 0;	
			КонецЕсли;
		КонецЦикла;
		Элементы.ГруппаТаблицыДопСодИнфГП.Видимость = Форма.ВариантПриемкиДоп > 0;
...
Функция КонтейнерОтображаемойВерсииРеквизита(Форма, ИмяЭлемента, ИндексТитула = "1") Экспорт
	...
	Иначе
		...
		Иначе
			...
			КонецЦикла;	
		КонецЕсли;
		НомерВерсии = Мин(НомерВерсии, МассивВерсий.ВГраница());
	КонецЕсли;		
...
Функция ПолучитьСтруктуруАдресаУчастника(ИмяРеквизита, Форма) Экспорт
	
	Префикс = СтрЗаменить(ИмяРеквизита, "Представление", "");
	
	КонтейнерРеквизитов = ПолучитьКонтейнерРеквизитов(Форма, ИмяРеквизита);
...
Процедура ЗаполнитьПредставлениеАдреса(ИмяРеквизита, Форма) Экспорт 
	
	СтруктураРеквизитовАдреса = ПолучитьСтруктуруАдресаУчастника(ИмяРеквизита, Форма);	
	...
	КонтейнерРеквизитовИлиОписание = ПолучитьКонтейнерРеквизитов(Форма, ИмяРеквизита);
...
Процедура ЗаполнитьРеквизитыАдреса(Форма, Результат, ИмяЭлемента) Экспорт
	
	Префикс = СтрЗаменить(ИмяЭлемента, "Представление", "");
	
	КонтейнерРеквизитов = ПолучитьКонтейнерРеквизитов(Форма, ИмяЭлемента);
...
Процедура ЗаполнитьРеквизитыПоСсылке(Элемент, Форма, ТекущиеДанные = Неопределено, ИмяТЧ = Неопределено) Экспорт
	...
	Если ТипЗнч(ЗначениеРеквизита) = ПолучитьТипОрганизация()
		...
		Если ИмяТЧ <> Неопределено Тогда
			ПрефиксРеквизитов = СтрЗаменить(ПрефиксРеквизитов, ИмяТЧ + "_", "");
		КонецЕсли;
