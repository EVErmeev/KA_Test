					...
					Если СтрокаОтгрузкиПоставки.Сумма > 0 И СтрокаОплаты.Сумма > 0 Тогда
						...
						Если СтрокаОплаты.ПорядокОперации >= ГлобальныеПеременные.ПорядокФакт Тогда
							...
							Если СтрокаОплаты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВзаимозачетЗадолженности
								...
								ИЛИ СтрокаОплаты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереносПлатежаМеждуФилиалами
...
Процедура ЗаполнитьДвиженияНУзаПериод(Организация, Период, ЭтоРасчетыСКлиентами, ДатаНачалаПересчета = Неопределено)
	...
	|			ТОГДА -Расчеты.СуммаНУ
	|			ИНАЧЕ Расчеты.СуммаНУ
	|		КОНЕЦ КАК ДолгРегл,
	|		ИСТИНА КАК ЭтоРасчетыСКлиентами
	|	ИЗ
	...
	|		И ТИПЗНАЧЕНИЯ(Расчеты.ДокументРегистратор) <> ТИП(Документ.РегламентнаяОперация)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		НаборЗаписейНУ.Отбор.Регистратор.Установить(Выборка.Регистратор);
		НаборЗаписейНУ.Записать();
		
		РасшифровкаРасчета = РегистрыСведений.РасчетПереоценкиВалютныхСредствНУ.СоздатьНаборЗаписей();
		РасшифровкаРасчета.Отбор.Регистратор.Установить(Выборка.Регистратор);
...
Функция РассчитатьСуммыНУ(Период, КурсНаДату, ЗаписьНУ, Оплата = Неопределено)
	...
	Если НачальныйОстатокДолга <> 0 Тогда
		...
		КонецЕсли;
		КурсоваяРазницаОстатка = Окр(КурсНаДату * ЗаписьНУ.ДолгОстаток - (НачальныйОстатокДолгаНУ +(-Оплата.ДолгРасход * КурсОстаткаНУ)), 2);
