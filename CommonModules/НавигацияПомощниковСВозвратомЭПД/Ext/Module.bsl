...
Функция НовыйСтруктураШага() Экспорт
	...
	СтруктураШага.Вставить("НомерВерсии",  1);
	...
	Для НомерШага = 1 По КоличествоШагов Цикл
		...
		Если СтруктураШага.КоличествоВерсий > 1 Тогда
			...
			Если ОтмечатьТекущий = Истина И НомерШагаСоСдвигом = ТекущийНомерШага Тогда
				Если СтруктураШага.НомерВерсии = 1 Тогда
				...
				МассивФорматированныхСтрок.Добавить(Новый ФорматированнаяСтрока(
														" " + Строка(СтруктураШага.НомерВерсии) + " из " + Строка(СтруктураШага.КоличествоВерсий) + " ", 
														ШрифтТекста, ЦветТекста));	
				Если СтруктураШага.НомерВерсии = СтруктураШага.КоличествоВерсий Тогда
			...
			КонецЕсли;
			ЭлементФормы.Высота = ЭлементФормы.Высота + 1;
		КонецЕсли;
