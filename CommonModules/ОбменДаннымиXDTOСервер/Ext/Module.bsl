...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока Не ПрерватьЦикл Цикл
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Отказ = Ложь;
			ИмяПланаОбмена = ПолучитьИмяПланаОбмена(КомпонентыОбмена, ПодтверждениеXDTO, Отказ);
...
Функция ПолучитьИмяПланаОбмена(КомпонентыОбмена, ПодтверждениеXDTO, Отказ)
	...
	ПланОбменаРаспознан =  Метаданные.ПланыОбмена.Найти(ИмяПланаОбмена) <> Неопределено;
		
	Если НЕ ПланОбменаРаспознан Тогда
		
		ВариантНастройки = "";
		Если ЗначениеЗаполнено(КомпонентыОбмена.УзелКорреспондента)
			И КомпонентыОбмена.УзелКорреспондента.Метаданные().Реквизиты.Найти("ВариантНастройки") <> Неопределено Тогда
			
			ВариантНастройки = КомпонентыОбмена.УзелКорреспондента.ВариантНастройки;
