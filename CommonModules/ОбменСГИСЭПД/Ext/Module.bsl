...
Процедура ПриЗаполненииНастройкиПоВидамЭлектронныхДокументов(Настройки) Экспорт
	...
	Для Каждого СтрНастройка Из Настройки Цикл
		Если СтрНастройка.СпособОбменаЭД <> Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО
...
Процедура ПриРаспознанииДокумента(ВидСообщения, ЭлементСхемы) Экспорт
	...
	Если ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН
		...
		Или ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ Тогда
...
Процедура ОтправитьQRРегламентноеЗадание() Экспорт
	...
	|	ЭлектроннаяТранспортнаяНакладная.ТитулГрузоотправителяТранспортнаяНакладнаяНомер КАК НомерЭТрН,
	|	ЭлектроннаяТранспортнаяНакладная.ТитулГрузоотправителяТранспортнаяНакладнаяДата КАК ДатаЭТрН,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектроннаяТранспортнаяНакладная КАК ЭлектроннаяТранспортнаяНакладная
	|		ПО ОбъектыУчетаДокументовЭДО.ОбъектУчета = ЭлектроннаяТранспортнаяНакладная.Ссылка
	|		И ЭлектроннаяТранспортнаяНакладная.РольУчастника = 3
	...
	|ИТОГИ
	|	МАКСИМУМ(Отправитель) КАК Отправитель,
	|	МАКСИМУМ(Токен) КАК Токен,
	|	МАКСИМУМ(ДатаНачалаПолучения) КАК ДатаНачалаПолучения,
	|	МАКСИМУМ(НомерЭТрН) КАК НомерЭТрН,
	|	МАКСИМУМ(ДатаЭТрН) КАК ДатаЭТрН,
	|	МАКСИМУМ(ЭлектронныйДокумент) КАК ЭлектронныйДокумент
	|ПО
	|	УИДМинтранс";
	...
	Если РезультатЗапроса.Пустой() = Ложь Тогда	
		...
		Пока ВыборкаУИДМинтранс.Следующий() Цикл	
			...
			СтруктураОтправки.Вставить("НомерДокумента", ВыборкаУИДМинтранс.НомерЭТрН);
			СтруктураОтправки.Вставить("ДатаДокумента", ВыборкаУИДМинтранс.ДатаЭТрН);
...
Функция ТекстЗаголовкаПисьмаQR() Экспорт
	
	Возврат "QR-код транспортной накладной №%1 от %2";
...
Функция ОтправитьПисьмаСQRКодами(МассивДляОтправки)
	...
	Для Каждого СтруктураОтправки Из МассивДляОтправки Цикл
		...
		Письмо.Тема = СтрШаблон(ТекстЗаголовкаПисьмаQR(), СтруктураОтправки.НомерДокумента, Формат(СтруктураОтправки.ДатаДокумента, "ДФ=dd.MM.yyyy;"));
...
Функция ТипыРегламентовЭПД() Экспорт
...
Функция ОтклонениеДоступно(ЭлементыРегламента, ЭтоВходящийЭДО, ТипЭлементаРегламентаУОУ) Экспорт
	...
	Возврат ЭтоВходящийЭДО Или ТипЭлементаРегламентаУОУ = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ОткГО;
...
Процедура ДополнитьЗапросВыборкиДокументовДляФормированияОтвета(ТекстыЗапросов, Параметры, ДляЭД = Ложь) Экспорт
	...
	Для К = 0 По ТекстыЗапросов.ВГраница() Цикл
		ТекстыЗапросов[К] = СтрЗаменить(ТекстыЗапросов[К], "ЭлектронныйДокументВходящийЭДО.ТребуетсяПодтверждение", 
...
// 
// Возвращаемое значение:
//  Соответствие - имя и значение реквизита
		...
		Если ЗначениеЗаполнено(СтруктураРеквизита.Значение) Тогда
			...
			Если ЗначениеЗаполнено(ПорядокРеквизита) Тогда
				...
				Если ПорядокРеквизита <= МассивПодстрокНаименования.Количество() Тогда
					МассивПодстрокНаименования[ПорядокРеквизита - 1] = ОформлениеРеквизита.До + СтруктураРеквизита.Значение + ОформлениеРеквизита.После;
				Иначе	 
					МассивПодстрокНаименования.Вставить(ПорядокРеквизита - 1, ОформлениеРеквизита.До + СтруктураРеквизита.Значение + ОформлениеРеквизита.После);
				КонецЕсли;	
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПорядокРеквизитаВПредставлении(ТипДанных, ИмяРеквизита)
	...
	ИначеЕсли ТипДанных = "ТитулГрузоотправителяТранспортноеСредство"
		...
		Или ТипДанных = "ТитулФрахтовщикаТранспортноеСредство" Тогда
		...
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяРеквизита, "Грузоподъемность") Тогда 
			Результат = 4;
		КонецЕсли;	
	ИначеЕсли ТипДанных = "Водитель" Тогда
	...
	ИначеЕсли ТипДанных = "ТитулПеревозчикаПрицеп" Тогда
		Если СтрЗаканчиваетсяНа(ИмяРеквизита, "ПрицепРегистрационныйНомер") Тогда
			Результат = 1;
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяРеквизита, "ПрицепМарка") Тогда 
			Результат = 2;
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяРеквизита, "ПрицепВместимость") Тогда 
			Результат = 3;
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяРеквизита, "ПрицепГрузоподъемность") Тогда 
			Результат = 4;
		КонецЕсли;
...
Процедура ОбработатьПараметры(Форма)
	...
	Если ЗначениеЗаполнено(ЗначениеЗаполнения)
		...
		ОписаниеХранимыхДанныхЭПД = ОбменСГИСЭПД.ПолучитьОписаниеХранимыхДанныхЭПДПоСсылке(ЗначениеЗаполнения, Форма.ГруппаДанных);
...
//			ЗаполнитьЗначенияСвойств(Форма, Параметры.ДанныеЗаполненияРеквизитовФормы, 
//											Параметры.ОписаниеРеквизитовФормы.ПараметрыФормыИзФормы);
		КонецЕсли;
...
Процедура СоздатьЭлементыОднострочногоВвода(Форма) Экспорт
	
	КоличествоТаблиц = Форма.Элементы.СтраницыТаблиц.ПодчиненныеЭлементы.Количество();
	Для НомерТаблицы = 1 По КоличествоТаблиц Цикл		
		ЭлементСтраницаТаб = Форма.Элементы.СтраницыТаблиц.ПодчиненныеЭлементы[НомерТаблицы - 1];
		ЭлементТаб = ЭлементСтраницаТаб.ПодчиненныеЭлементы[0];		
		ТаблицаДанные = Форма[ЭлементТаб.ПутьКДанным];
		...
		Иначе
			НоваяСтрока = ТаблицаДанные[0];
		КонецЕсли;
		Колонки = Новый Структура(Форма.ОписаниеРеквизитовФормы.ОписаниеТаблицФормы[ЭлементТаб.ПутьКДанным]);
		Для Каждого КиЗ Из Колонки Цикл
			Если СтрНачинаетсяС(КиЗ.Ключ, "Заполнить") Тогда
				НоваяСтрока[КиЗ.Ключ] = "Заполнить";
			КонецЕсли;
		КонецЦикла;			
		ИмяСтраницы = СтрЗаменить(ЭлементСтраницаТаб.Имя, "Страница", "СтраницаОднострочная");
		СтраницаОднострочная = Форма.Элементы.Добавить(ИмяСтраницы, Тип("ГруппаФормы"), Форма.Элементы.СтраницыТаблиц);
		СтраницаОднострочная.Вид = ВидГруппыФормы.Страница;
		МассивПолейЗаполнить = Новый Массив;		
		ДобавитьОднострочныеПоляРекурсивно(Форма, ЭлементТаб, СтраницаОднострочная, МассивПолейЗаполнить);
		ЭлементСтраницаТаб.Видимость = Ложь;
	КонецЦикла;
...
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект.ЭлектроннаяСопроводительнаяВедомость, ДокументОбъект.ЭлектроннаяТранспортнаяНакладная, ДокументОбъект.ЭлектронныйЗаказНаряд - ДокументОбъект
//  МетаданныеОбъекта - ОбъектМетаданныхДокумент - Метаданные объекта
// 
// Возвращаемое значение:
//  Массив - описание хранимых данных объекта
Процедура ЗаполнитьПредставлениеХранимыхДанныхОбъекта(ДокументОбъект, ИмяМетаданных, МакетСоответствиеИмен) Экспорт
		...
		Для Каждого ИмяТЧ Из ОписаниеГруппыДанных.Таблицы Цикл
			Если ИмяТЧ = "" Тогда
				...
				Для Каждого	ИмяРеквизита Из ОписаниеГруппыДанных.Реквизиты Цикл
					ЗначениеРеквизита = Неопределено;
					Если ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Свойство(ИмяРеквизита, ЗначениеРеквизита) Тогда
				...
				Если ЕстьХранимыеДанные = Истина Тогда
					ПредставлениеХранимыхДанных = ОбменСГИСЭПД.ПредставлениеХранимыхДанныхЭПД(МассивРеквизитов, 
																ИмяМетаданных, 
																КиЗ.Ключ);
			...
			Иначе
				...
				Пока ЕстьХранимыеДанные = Истина Цикл 
					ЕстьХранимыеДанные = Ложь;
					Для Каждого	ИмяРеквизита Из ОписаниеГруппыДанных.Реквизиты Цикл
						КлючСтруктуры = ИмяТЧ + "__" + К + "__" + ИмяРеквизита;
						ИмяВМакете = ИмяТЧ + "." + ИмяРеквизита;
						ЗначениеРеквизита = Неопределено;
						Если ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Свойство(КлючСтруктуры, ЗначениеРеквизита) Тогда
							Если ЗначениеРеквизита = Неопределено Тогда
								ЗначениеРеквизита = "";
							КонецЕсли;
							ПредставлениеРеквизита = КэшПредставлений.Получить(ИмяРеквизита);
					...
					ПредставлениеХранимыхДанных = ОбменСГИСЭПД.ПредставлениеХранимыхДанныхЭПД(МассивРеквизитов, 
																...
																КиЗ.Ключ);
					ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
						ИмяТЧ + "__" + К + "__" + ПрефиксХранимыхДанных + КиЗ.Ключ,
						ПредставлениеХранимыхДанных);
					К = К + 1;
				КонецЦикла;
			КонецЕсли;	
	...
	Если ЗначениеЗаполнено(Участник) Тогда
		...
		Иначе
			Если Строка(ЮрФизЛицо) = "Физическое лицо" Тогда
				...
				Результат.Вставить("ИЛ_ИНН", СведенияОбУчастнике.ИНН);
		...
		// Результат.ОбластьКрай = АдресОтправителяСтруктура.Регион + ?(АдресОтправителяСтруктура.РегионТипКраткий = "", "", " ") + АдресОтправителяСтруктура.РегионТипКраткий;
...
Процедура СоздатьЭлементыВводаЗначенийСписка(Форма, ИмяТаблицы, ИмяПоля, ДобавитьСтроку = Ложь, ДобавлениеСверху = Ложь) Экспорт
	...
	Для Каждого РеквизитКолонки Из КолонкиТаблицы Цикл
		Если РеквизитКолонки.Имя = ИмяПоля Тогда
			НайденныйРеквизит = РеквизитКолонки;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если НайденныйРеквизит <> Неопределено Тогда
		...
		Для К = ИндексОт По ИндексДо Цикл
			ГУИД = Строка(К); 
			ИтераторСтрокой = СтрЗаменить(Строка(К), Символы.НПП, "");
			
			НоваяГруппа = Форма.Элементы.Добавить("Группа" + ИмяТаблицы + ИмяПоля + "_" + ГУИД, Тип("ГруппаФормы"), ГруппаПоТаблице); 
			...
			НовыйЗаголовок = Форма.Элементы.Добавить("Заголовок" + ИмяТаблицы + ИмяПоля + "_" + ГУИД, Тип("ДекорацияФормы"), НоваяГруппа);
			НовыйЗаголовок.Вид = ВидДекорацииФормы.Надпись;
			НовыйЗаголовок.Заголовок = ЗаголовокПоля + ?(К > 0, " " + Строка(К + 1), "") + ":";
			//НовыйЗаголовок.Ширина = ШиринаЗаголовка;
			
			НовыйРеквизит = Форма.Элементы.Добавить(ИмяТаблицы + "__" + ИмяПоля + "__" + ГУИД, Тип("ПолеФормы"), НоваяГруппа);
			...
			Если К = 0 Тогда
				КомандаДобавления = Форма.Команды.Найти("ДобавлениеПоляВвода" + ИмяТаблицы + "__" + ИмяПоля); 
				Если КомандаДобавления = Неопределено Тогда
					КомандаДобавления = Форма.Команды.Добавить("ДобавлениеПоляВвода" + ИмяТаблицы + "__" + ИмяПоля);
				...
				КонецЕсли;			
				НоваяКнопка = Форма.Элементы.Добавить("КнопкаДобавить" + ИмяТаблицы + ИмяПоля + "_" + ГУИД, Тип("КнопкаФормы"), НоваяГруппа);
...
Процедура СохранитьДанныеОбъектаВБД(СтрокаДляЗагрузки, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект) Экспорт
	...
	Если СтрокаДляЗагрузки.ВидЭД = Перечисления.ТипыДокументовЭДО.ЭТрН
		...
		Или СтрокаДляЗагрузки.ВидЭД = Перечисления.ТипыДокументовЭДО.ЭСВ Тогда	
...
Функция НайтиСоздатьЭПД(ДеревоДляСовместимостиСБЭД, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект) Экспорт
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ Док.Ссылка ИЗ Документ." + ИмяДокумента + " КАК Док 
								...
								|	И ВЫРАЗИТЬ(Док." + КлючеваяИнформацияПоТитулу.ПолеПоиска + " КАК СТРОКА(200))  = &ЗначениеПоиска" + ДопОтбор;
		...
		Если ЗначениеЗаполнено(СсылкаПоИдФайла) Тогда
			...
			Если КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
				...
				Если ЗначениеЗаполнено(ДокументОбъект.ТитулПеревозчикаВыдачаИдентификаторФайла) Тогда
					...
					ИначеЕсли КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8 Тогда
					...
					ИначеЕсли КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул3 Тогда
					...
					ИначеЕсли КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул4 Тогда
	...
	Если КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН Тогда
	...
	ЗаполнитьПредставлениеХранимыхДанныхОбъекта(ДокументОбъект, ДокументОбъект.Метаданные().ПолноеИмя(), ОбластьПоискаИмен);
	...
	Если КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьТекущийДоступныйТитулЭТрН(ДокументОбъект);
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьТекущийДоступныйТитулЭЗН(ДокументОбъект);
	ИначеЕсли КлючеваяИнформацияПоТитулу.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьТекущийДоступныйТитулЭСВ(ДокументОбъект);
	КонецЕсли;
	
	ДокументОбъект.ДополнительныеСвойства.Вставить("НомерВерсии", КлючеваяИнформацияПоТитулу.НомерВерсии);

	ВерсияТитула = Новый Структура;
	ВерсияТитула.Вставить("НомерВерсии", КлючеваяИнформацияПоТитулу.НомерВерсии);
...
Функция ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, ПрефиксРеквизитов)
...
Процедура ЗаполнитьПредставленияСсылочныхТипов(ДокументОбъект)
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектронныйЗаказНаряд") Тогда
		...
		// По таблице "Сведения об опасном грузе"
		К = 1;
		КлючСтруктуры = "ТитулФрахтователяСведенияОбОпасномГрузе__" + К + "__НадлежащееОтгрузочноеНаименование";
		НадлежащееОтгрузочноеНаименование = Неопределено;
		Пока СтруктураРеквизитов.Свойство(КлючСтруктуры, НадлежащееОтгрузочноеНаименование) Цикл
			Если ЗначениеЗаполнено(НадлежащееОтгрузочноеНаименование) Тогда
				Если СтрДлина(НадлежащееОтгрузочноеНаименование) <= 150 Тогда
				...
				КонецЕсли;
				КлассификаторДОПОГНайдено = Справочники.КлассификаторДОПОГЭПД.НайтиПоНаименованию(КраткоеНаименованиеДОПОГ);
				КлючЗначенияСсылки = "ТитулФрахтователяСведенияОбОпасномГрузе__" + К + "__СсылкаДОПОГ";
				Если ЗначениеЗаполнено(КлассификаторДОПОГНайдено) Тогда		
					СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, КлассификаторДОПОГНайдено);
				Иначе
					СтруктураРеквизитов.Вставить(КлючЗначенияСсылки, НайтиСоздатьСтроковойЗначениеЭПД(КраткоеНаименованиеДОПОГ));	
...
Функция ТипДокументаОтправляетсяВГИСЭПД(ТипДокумента) Экспорт
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН
		Или ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура УстановитьТекущийШаг(ДокументОбъект) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектронныйЗаказНаряд") Тогда
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектроннаяСопроводительнаяВедомость") Тогда
...
Функция РеквизитыТекущейВерсииТитула(Объект, Титул, Заполнить = Ложь) Экспорт
	...
	ИначеЕсли Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН Тогда
		МетаданныеОбъекта = Метаданные.Документы.ЭлектронныйЗаказНаряд;
	ИначеЕсли Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ Тогда
		МетаданныеОбъекта = Метаданные.Документы.ЭлектроннаяСопроводительнаяВедомость;
	КонецЕсли;
...
Функция КартаТаблиц(ДанныеФормирования, ПрефиксТитула, 
			...
			ТолькоЗаполненные = Истина, ПолучитьСписокКолонок = Ложь) Экспорт
...
Функция ДанныеРеквизитовЭПД(Документ, Титул = Неопределено) Экспорт
	...
	Если ВыборкаНомерВерсии.Следующий() Тогда
		...
		Пока ВыборкаИмяТЧ.Следующий() Цикл
			Если ВыборкаИмяТЧ.ИмяТабличнойЧасти = "" Тогда
				...
				Пока Выборка.Следующий() Цикл
					Если Выборка.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.СсылочныйТип Тогда
						Результат.Вставить(Выборка.ИмяРеквизита, Выборка.ЗначениеРеквизитаСсылка);
					ИначеЕсли Выборка.ТипРеквизита = Перечисления.ТипыРеквизитовЭПД.НеограниченнаяСтрока Тогда
						Результат.Вставить(Выборка.ИмяРеквизита, Выборка.ЗначениеРеквизитаСтрока);	
					Иначе
...
Функция УстановитьСоединениеССерверомИнтернета(URLСервера, ОписаниеОшибки = "", Таймаут = 60) Экспорт
	...
	Исключение
		...
		ОбменСГИСЭПДВызовСервера.ЗаписатьОшибкуВЖурнал(НСтр("ru = 'Установление соединения с сервером интернета'"), ИнформацияОбОшибке);
...
Функция ПолучитьИдентификаторЧерезСервисОператораЭДО(ИдентификаторАбонента, ЭтоВторойТитул = Ложь, ДокументСсылка = Неопределено) Экспорт
	...
	Если ЭтоВторойТитул = Истина И ДокументСсылка <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ЭД) Тогда
			...
			АдресРесурса = АдресРесурса + "&transaction=ShipperInformation";
		КонецЕсли;	
	КонецЕсли;
	ЗапросHTTP = Новый HTTPЗапрос(АдресРесурса);
...
Функция ДанныеСертификатаУчетнойЗаписиЭДО(ИдентификаторУчетнойЗаписи) Экспорт
	...
	|ГДЕ
	|	СертификатыУчетныхЗаписей.ИдентификаторЭДО = &ИдентификаторУчетнойЗаписи
	|	И РАЗНОСТЬДАТ(&ТекущаяДата, СертификатыУчетныхЗаписей.ДействителенДо, МИНУТА) > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	СертификатыУчетныхЗаписей.ДействителенДо";
...
Функция УзлыСхемыФормыТранспортнойНакладной(ИмяФормы)
	...
	ИначеЕсли ИмяФормы = "ТитулГрузоотправителятСторонаОснованияРасчетовИнымЛицом" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфГО/СвГО/ОснРасчИным/ИдРекСост");
	...
	ИначеЕсли ИмяФормы = "ТитулГрузоотправителяРеквизитыГОКакВладелецаПунктаПогрузки" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфГО/СвПогруз/ВладИнфр/ИдентРекГО");
	...
	ИначеЕсли ИмяФормы = "ТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон" Тогда
	...
	ИначеЕсли ИмяФормы = "ТитулПеревозчикаЗаменыТранспортноеСредство" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфЗамен/ЗаменТС/ТС");
...
Функция ОсобоеОформлениеДляЗаполненногоЭлементаЭТрН(ПутьКЭлементуФормы, ОбъектСДаннымиДокумента, СтруктураРеквизитов)
	...
	ИначеЕсли ПутьКЭлементуФормы = "ЗаполнитьТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон" Тогда
	
		Заголовок = ПолучитьКоличествоОбъектовТабЧасти(ОбъектСДаннымиДокумента,
			"ТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон",
			"сторона",
			"",
			СтруктураРеквизитов);
		Возврат Заголовок;		 

	ИначеЕсли ПутьКЭлементуФормы = "ЗаполнитьДопТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон" Тогда
	
		Заголовок = ПолучитьКоличествоОбъектовТабЧасти(ОбъектСДаннымиДокумента,
			"ДопТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон",
			"сторона",
			"",
			СтруктураРеквизитов);
		Возврат Заголовок;		 
...
Функция ПроверитьЗаполнениеДокументаЭПД(ДанныеДляПроверки, ТекущийТитул, ВидДокумента) Экспорт
	...
	Для НомерСтроки = 1 По КоличествоСтрокМакета Цикл
		...
		Если ЭтоДопТитул = Истина Тогда
			...
			Для Каждого ИмяПоля Из МассивПолей Цикл
				Если СтрНачинаетсяС(ИмяРеквизита, "Титул") Тогда
					МассивПолейДоп.Добавить("Доп" + ИмяПоля);
				Иначе
					МассивПолейДоп.Добавить(ИмяПоля);		
				КонецЕсли;
			КонецЦикла;	
	...
	Для НомерСтроки = 1 По КоличествоСтрокМакета Цикл
		...
		Если ЧастиИмени.Количество() > 1 
			Или ЧастиИмениСледующего.Количество() > 1 
			Или ЗначениеЗаполнено(ОписаниеРеквизита) = Ложь Тогда
			Продолжить;
		КонецЕсли;
		Узел = МакетСоответствиеИмен.Область(НомерСтроки, 2).Текст;
		УзелОбязателен = Ложь;
