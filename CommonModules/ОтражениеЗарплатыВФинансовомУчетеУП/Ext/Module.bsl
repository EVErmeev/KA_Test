...
Функция РаспределитьРасходыНаОплатуТрудаПоВыработке(ПараметрыЗапроса, ТекстыЗапроса) Экспорт
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВТРасходыРаспределяемые", ТекстыЗапроса) Тогда
...
Функция ТекстЗапросаБазаРаспределения()
	...
	|	ТрудозатратыНЗП.Количество КАК Количество,
	...
	|
	|	ВЫБОР
	|		КОГДА Трудозатраты.БазаРаспределения = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.НормативыОплатыТруда)
	|			ТОГДА Трудозатраты.НормативнаяСтоимость
	|		КОГДА Трудозатраты.БазаРаспределения = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ДлительностьВыполненияРабот)
	|			И ЕСТЬNULL(ВидыРабот.КратностьТрудоемкости, 0) > 0
	...
	|			ТОГДА Трудозатраты.НормативнаяСтоимость
	|		КОГДА Трудозатраты.БазаРаспределения = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ДлительностьВыполненияРабот)
	|			И ВидыРабот.КратностьТрудоемкости > 0
	|			ТОГДА ВЫРАЗИТЬ(Трудозатраты.Количество * ВидыРабот.Трудоемкость / ВидыРабот.КратностьТрудоемкости КАК ЧИСЛО(15,3))
	|		ИНАЧЕ 0
	|	КОНЕЦ > 0";
