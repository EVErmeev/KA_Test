...
Функция ЗапросДанных(ДокументИмя, ЭтоОбъектРасчетов = Истина, ВременныеТаблицыДокументаПодготовлены = Ложь) Экспорт
	...
	Иначе
		...
		ТекстыСозданияВременныхТаблиц.Добавить(РасчетСебестоимостиНДС.ТекстЗапросаДетализацияПартийНДС(,,, СтруктураОтбора) + ";" + Символы.ПС);
...
Функция ЗапросОчистки(ИмяДокумента) Экспорт
	...
	Если ИмяДокумента <> "РегистраторРасчетов" Тогда
		ВТЗапросаДанных = РеглУчетВыборкиСерверПовтИсп.ВТЗапроса(РасчетСебестоимостиНДС.ТекстЗапросаДетализацияПартийНДС());
		Для каждого ВТ Из ВТЗапросаДанных Цикл
			ТекстЗапросаОчистки = ТекстЗапросаОчистки + "
				|УНИЧТОЖИТЬ " + ВТ + ";";
		КонецЦикла;
...
#Область ВтСтроки
	...
	|		КОНЕЦ) КАК СуммаНУ,
	|	ВЫБОР
	|		КОГДА НЕ (&ПартионныйУчетВерсии22 И Движения.Период >= НАЧАЛОПЕРИОДА(&ДатаПереходаНаПартионныйУчетВерсии22, МЕСЯЦ))
...
Функция ТекстПартии() Экспорт
	...
	ТекстЗапроса =
		...
		|	ВЫБОР КОГДА Т.ЭтоРаспределениеПрочихЗатрат
		|		ТОГДА
		|			ВЫБОР КОГДА Т.КорПодразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
...
Функция ТекстВТВыручкаИСебестоимостьПродаж()
	...
	|	Строки.НастройкаХозяйственнойОперации КАК НастройкаХозОперации,
...
Функция ПараметрыОтраженияРеглУчетаПоУмолчанию(ДетализацияПроводок = Ложь) Экспорт
	...
	РасчетСебестоимостиНДС.ДополнитьСтруктуруПараметровДляДетализацииПартийНДС(СтруктураПараметров, Истина);
