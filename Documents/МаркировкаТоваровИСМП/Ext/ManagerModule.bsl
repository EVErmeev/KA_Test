...
#Область РазборКодовМаркировки
	...
	Если ЭтоПиво Тогда
		...
		// Настройки ЕГАИС
		ОбъемМаркируемойПродукции = ИнтеграцияИСМП.ОбъемМаркируемойПродукцииВЛитрах(НоменклатураДокумента);
		// Настройки ИСМП
	КонецЕсли;
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
		...
		Если ТребуетсяПередатьКодМаркировки Тогда
			...
			Если ЭтоПиво Тогда
				...
				Объем = ОбъемМаркируемойПродукции[СтрокаТЧМаркированныеТовары.Номенклатура];
				// Объем заполнен в подсистеме ЕГАИС
				Если Объем <> Неопределено И Объем > 0 Тогда
				...
				// Если требует взвешивания, объем перевести надо в литры
				ИначеЕсли СтрокаТЧМаркированныеТовары.ТребуетВзвешивания Тогда
					СтрокаТЧМаркированныеТовары.Количество = СтрокаТЧМаркированныеТовары.Количество
						* ИнтеграцияИСВызовСервера.ОбъемУпаковки(СтрокаТЧМаркированныеТовары.Номенклатура,,КэшированныеЗначения);
				КонецЕсли;
			КонецЕсли;
...
#КонецОбласти
	...
	Если ЭтоБАД Или ЭтоПиво Или ЭтоБАПиво Или ЭтоУпакованнаяВода Тогда
...
Функция ОтчетОНанесенииОтчетПроизводственнойЛинииJSON(РезультатЗапроса, ДополнительныеПараметры, Повторно)
	...
	РазбиватьПоСрокамГодности = ЭтоМолочнаяПродукция Или ЭтоБАД Или ЭтоПиво Или ЭтоБАПиво;
...
#КонецОбласти
	...
	Если ЭтоПиво Тогда
		...
		Если ЗначениеЗаполнено(Шапка.Номенклатура) Тогда
			ОбъемМаркируемойПродукции = ИнтеграцияИСМП.ОбъемМаркируемойПродукцииВЛитрах(Шапка.Номенклатура);
			Объем = ОбъемМаркируемойПродукции[Шапка.Номенклатура];
			Если Объем <> Неопределено И Объем > 0 Тогда
...
Функция СообщенияJSONОтчетОНанесении(Шапка, ДополнительныеПараметры)
	...
	Для Каждого ДанныеСообщения Из ДополнительныеПараметры.ГруппыКодов Цикл
		...
		ИначеЕсли ЭтоКреслаКоляски Тогда
			ТелоЗапроса.Вставить("productionDate", Формат(Шапка.ДатаПроизводства,"ДФ=dd.MM.yyyy"));
		КонецЕсли;
