...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийЗЕРНО.АннулированиеСДИЗРасчетСтатуса
		Или Операция = Перечисления.ВидыОперацийЗЕРНО.АннулированиеСДИЗ Тогда
...
Функция СформироватьПартииЗЕРНО(ДокументСсылка, ДополнительныеПараметры)
	...
	Для Каждого СтрокаПартии Из Партии Цикл
		...
		Иначе
			...
			ИсточникДанныхПроизводитель = Неопределено;
			ДанныеСопоставления = Справочники.КлючиРеквизитовОрганизацийЗЕРНО.ОрганизацииКонтрагентыПоКлючам(Производитель).Получить(Производитель);
			Если ДанныеСопоставления <> Неопределено Тогда
				
				ОрганизацияКонтрагент = Неопределено;
				
				ТаблицаИсточникиРеквизитов = ИнтеграцияЗЕРНО.НоваяТаблицаОрганизацияКонтрагентПодразделение();
				Если ЗначениеЗаполнено(ДанныеСопоставления.Организация) Тогда
					ОрганизацияКонтрагент = ДанныеСопоставления.Организация;
				ИначеЕсли ЗначениеЗаполнено(ДанныеСопоставления.Контрагент) Тогда
					ОрганизацияКонтрагент = ДанныеСопоставления.Контрагент;
				КонецЕсли;
				
				Если ОрганизацияКонтрагент <> Неопределено Тогда
					ИнтеграцияЗЕРНО.ДобавитьВТаблицуОтбораОрганизациюПодразделение(
						ТаблицаИсточникиРеквизитов, ДанныеСопоставления.Организация, ДанныеСопоставления.Подразделение);
					ИнтеграцияЗЕРНОВызовСервера.РеквизитыОрганизацийКонтрагентов(ТаблицаИсточникиРеквизитов);
					ИсточникДанныхПроизводитель = ИнтеграцияЗЕРНО.РеквизитыОрганизацииКонтрагента(
						ТаблицаИсточникиРеквизитов, ДанныеСопоставления.Организация, ДанныеСопоставления.Подразделение);
				КонецЕсли;
			...
			Если ИсточникДанныхПроизводитель = Неопределено Тогда
				...
				ИнтеграцияЗЕРНОКлиентСервер.ДобавитьТекстОшибки(
					АбстрактноеСообщениеXML,
					СтрШаблон(
						НСтр("ru = 'В строк %1 не найдена организация/контрагент по ключу %2'"),
						СтрокаПартии.НомерСтроки,
						Производитель));
					Продолжить;
			Иначе
...
Функция ДанныеКонтрагентовОрганизацийПриОформленииСДИЗ(Шапка, Перевозчики)
...
Функция ДополнительныеСведенияИмпортЭкспортXDTO(ДанныеЗаполнения, ЭлементСДИЗXDTO, ПространствоИмен, СообщениеXML)
	...
	// В текущей схеме нет ревизитов для НомерПротоколаИспытаний, ДатаПротоколаИспытаний, НомерТаможеннойДекларации
