...
Процедура ОтправитьQR(Команда)
	...
	Иначе
		...
		ОткрытьФорму("Документ.ЭлектроннаяТранспортнаяНакладная.Форма.ОтправкаQR", 
						ПараметрыФормы, 
						ЭтотОбъект, 
	...
	Если ЗначениеЗаполнено(Объект.ТекущийТитул) Тогда
		...
		Пока СтруктураВерсииТитула.Свойство(КлючОткуда, ЗначениеОткуда) Цикл
			...
			И ЗначениеОткуда <> Объект.ИдентификаторПеревозчика Тогда
				...
				ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, КлючКуда, ЗначениеОткуда, , КонтейнерОтображаемойВерсии);	
				К = К + 1;
				КлючОткуда = "ТитулГрузоотправителяИдентификаторыИныхПолучателейФайла__" + К + "__" + "ИдентификаторИногоПолучателя";
			КонецЕсли;
		КонецЦикла;
...
&НаКлиенте
Процедура ЗаполнитьТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон(Команда)
...
&НаКлиенте
Процедура ЗаполнитьДопТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон(Команда)
...
Функция НавигацияПомощника()
	...
	Если СтатусПереадресовки(ЭтотОбъект) > 0 Тогда
		НомерШага = ?(ПереадресовкаДоЗамены(ЭтотОбъект), 3, 4);
	...
	Если СтатусЗамены(ЭтотОбъект) > 0 Тогда
		НомерШага = ?(ПереадресовкаДоЗамены(ЭтотОбъект), 4, 3);
...
Процедура ОбработатьИзменениеШага(ИмяШага, НавигационнаяСсылка)
	...
	Если ИмяШага = "Шаг9" Или ИмяШага = "Шаг10" Или ИмяШага = "Шаг11" Или ИмяШага = "Шаг12" Тогда
		...
		Если ИмяШага = "Шаг9" Тогда
			...
			УстановитьВидимостьСтраницыНачалоДоп();
		ИначеЕсли ИмяШага = "Шаг10" И СтатусЗаменыДоп(ЭтотОбъект) > 0 Тогда
...
&НаКлиенте
Процедура ЗаполнитьИдентификациюФайлаОбмена(ИнформацияПоПрефиксамТитула, ИдентификаторОтправителя, Переформировать = Ложь)
	...
	Если ТребуетсяЗаполнить = Ложь Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул3") Тогда	
		...
		Иначе
			...
			КоличествоГрузовыхМестОтправителя = 
				ОбменСГИСЭПДКлиентСервер.СуммаПоСтрокамТаблицыИзСтруктурыФормы(ЭтотОбъект,
					"ТитулГрузоотправителяГрузы", "КоличествоГрузовыхМест");
			...
			Если ТитулГрузополучателяОбщееКоличествоПринятыхГрузовыхМест < КоличествоГрузовыхМестОтправителя Тогда
				СодержаниеОперацииПримеГруза = "Частичная выгрузка";	
			Иначе	
				СодержаниеОперацииПримеГруза = "Лицом, управомоченным на получение груза, при указанных обстоятельствах принят от перевозчика груз с указанными характеристиками";
			КонецЕсли;
			
			ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ТитулГрузополучателяСодержаниеОперацииПриемГруза",
				СодержаниеОперацииПримеГруза, , КонтейнерОтображаемойВерсии);
		КонецЕсли;
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул3") Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ДопТитулГрузополучателяДатаФормированияФайла", ДатаФормирования, , КонтейнерОтображаемойВерсии); 
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ДопТитулГрузополучателяВремяФормированияФайла", ВремяФормирования, , КонтейнерОтображаемойВерсии);
		Если ЗначениеЗаполнено(ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(ЭтотОбъект, 
				"ДопТитулГрузополучателяПричинаОтказаВПриемеГруза", , КонтейнерОтображаемойВерсии)) Тогда
			ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ДопТитулГрузополучателяСодержаниеОперацииОтказВПриемеГруза", 
				"Лицом, управомоченным на получение груза, при указанных обстоятельствах отказано в принятии от перевозчика груза с указанными характеристиками", , КонтейнерОтображаемойВерсии);	
		Иначе
			ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ДопТитулГрузополучателяСодержаниеОперацииПримеГруза", 
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул4") Тогда
		...
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ТитулПеревозчикаВыдачаИдентификаторФайлаГрузополучателя", Объект.ТитулГрузополучателяИдентификаторФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ТитулПеревозчикаВыдачаДатаФормированияФайлаГрузополучателя", Объект.ТитулГрузополучателяДатаФормированияФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ТитулПеревозчикаВыдачаВремяФормированияФайлаГрузополучателя", Объект.ТитулГрузополучателяВремяФормированияФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул4") Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				...
				"Груз с указанными характеристиками сдан водителем, уполномоченным перевозчиком на перевозку груза, лицу, управомоченному на получение груза", , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ДопТитулПеревозчикаВыдачаИдентификаторФайлаГрузополучателя", Объект.ДопТитулГрузополучателяИдентификаторФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ДопТитулПеревозчикаВыдачаДатаФормированияФайлаГрузополучателя", Объект.ДопТитулГрузополучателяДатаФормированияФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
				ЭтотОбъект, "ДопТитулПеревозчикаВыдачаВремяФормированияФайлаГрузополучателя", Объект.ДопТитулГрузополучателяВремяФормированияФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8") Тогда		
		...
		Иначе
			...
			ОбменСГИСЭПДКлиентСервер.УдалитьСтрокиТаблицыИзСтруктурыФормы(
				ЭтотОбъект, "ТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон", , КонтейнерОтображаемойВерсии);
		КонецЕсли;
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул8") Тогда
		...
		Иначе
			...
			ОбменСГИСЭПДКлиентСервер.УдалитьСтрокиТаблицыИзСтруктурыФормы(
				ЭтотОбъект, "ДопТитулПеревозчикаЗаменыДокументОтражающийПричиныЗаменыРеквизитыСторон", , КонтейнерОтображаемойВерсии);
		КонецЕсли;
	...
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул6") Тогда
		...
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
			ЭтотОбъект, "ТитулГрузоотправителяФХЖИдентификаторФайлаПеревозчика", Объект.ТитулПеревозчикаФХЖИдентификаторФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
			ЭтотОбъект, "ТитулГрузоотправителяФХЖДатаФайлаПеревозчика", Объект.ТитулПеревозчикаФХЖДатаФормированияФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
			ЭтотОбъект, "ТитулГрузоотправителяФХЖВремяФайлаПеревозчика", Объект.ТитулПеревозчикаФХЖВремяФормированияФайла, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
	...
	Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1")
		И НЕ ЗначениеЗаполнено(Объект.УИДМинтранс) Тогда
		ПолучитьГУИДОтОператора(Объект.ИдентификаторГрузоотправителя);
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул2")
		И НЕ ЗначениеЗаполнено(Объект.УИДМинтранс) Тогда
		ПолучитьГУИДОтОператора(Объект.ИдентификаторПеревозчика, Истина);
	КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ДанныеСертификатаУчетнойЗаписиЭДО.НомерДоверенности) Тогда
			КодСтатуса = "2";
		Иначе
			ТитулГрузоотправителяПредставительГрузоотправителя = ОбменСГИСЭПДКлиентСервер.ПолучитьЗначениеРеквизитаИзСтруктурыФормы(ЭтотОбъект, 
																"ТитулГрузоотправителяПредставительГрузоотправителя", , КонтейнерОтображаемойВерсии);	
			Если ЗначениеЗаполнено(ТитулГрузоотправителяПредставительГрузоотправителя) Тогда
				КодСтатуса = "3";	
			КонецЕсли;
		КонецЕсли;
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
			ЭтотОбъект, ИнформацияПоПрефиксамТитула.ВПрограмме + "ПодписантКодСтатуса", КодСтатуса, , КонтейнерОтображаемойВерсии);
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(
...
Процедура ОтправитьЧерезЭДОПослеЗаписи(Повторно = Ложь)
	...
	КонецЕсли;
	ИнформацияПоПрефиксамТитула = ОбменСГИСЭПДКлиентСервер.ПрефиксТитулаПоЭлементуРегламентаЭДО(Объект.ТекущийТитул);
	ЗаполнитьИдентификациюФайлаОбмена(ИнформацияПоПрефиксамТитула, ИдентификаторОтправителя(), Повторно);	
	ПараметрыЗаписи = Новый Структура;
	ПараметрыЗаписи.Вставить("НеТребуетсяИдентификацияФайла");
	Записать(ПараметрыЗаписи);
	...
	ОчищатьТаблицу = Истина; 
	Если ОбменСГИСЭПДКлиент.ЗаполнитьТаблицуОшибокЭПД(Объект.Ссылка, ОшибкиЗаполнения, ОчищатьТаблицу) = Истина Тогда	
		Элементы.ГруппаОшибки.Видимость = Истина;
		Элементы.НадписьОшибки.Заголовок = "Ошибки заполнения (" + Строка(ОшибкиЗаполнения.Количество()) + ")";
		Доступность = Истина;
