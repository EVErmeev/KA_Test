...
Процедура ПолучитьДанныеПакета()
	...
	СостоянияДокументовЭДО = ПакетыДокументовЭДО.СостоянияДокументовПакета(ИдентификаторПакета);
	СостояниеПакета = ПакетыДокументовЭДО.СостояниеПакета(СостоянияДокументовЭДО);
	...
	Если ЗначениеЗаполнено(СостоянияДокументовЭДО) Тогда
		ДоступноИзменениеПакета = ПакетыДокументовЭДО.ДоступноИзменениеПакета(СостоянияДокументовЭДО);
	Иначе
		СостоянияДокументовЭДО = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СостояниеЭДО);
		ДоступноИзменениеПакета = ПакетыДокументовЭДО.ДоступноИзменениеПакета(СостоянияДокументовЭДО);
	КонецЕсли;
...
Процедура ОбернутьДанныеСообщенияВПДФ(Действие)
	...
	Если ЭтоАдресВременногоХранилища(АдресПДФКонтейнера) Тогда
		ПДФКонтейнер = ПолучитьИзВременногоХранилища(АдресПДФКонтейнера);
		...
		Вложения.Добавить(
			ОписаниеСообщения.Данные.Документ.ИмяФайла, ОписаниеСообщения.Данные.Документ.ДвоичныеДанные, ,
			ТипСвязиВложенияPDF.Данные
		);
		
		ПотокПДФ = Новый ПотокВПамяти;
		ЗаписьДанных = Новый ЗаписьДанных(ПотокПДФ);
		ЗаписьДанных.Записать(ПДФКонтейнер);
		ЗаписьДанных.Закрыть();
	
		ЗаписьПДФ = Новый ЗаписьPDF;
		ЗаписьПДФ.Открыть(ПотокПДФ);
		ЗаписьПДФ.ЗаписатьВложения(Вложения);
		ЗаписьПДФ.Закрыть();
		
		НовыеДанныеСообщения = ПотокПДФ.ЗакрытьИПолучитьДвоичныеДанные();
		ОписаниеСообщения.Данные.Документ.ИмяФайла = СтрЗаменить(ОписаниеСообщения.Данные.Документ.ИмяФайла, ".xml", ".pdf");
		ОписаниеСообщения.Данные.Документ.ДвоичныеДанные = НовыеДанныеСообщения;
		
		Параметры.РезультатПодготовкиДанных.АдресОписанияСообщения = ПоместитьВоВременноеХранилище(ОписаниеСообщения, Параметры.РезультатПодготовкиДанных.АдресОписанияСообщения);
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьВложениеИзФайлаНаДискеЗавершить(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	...
	Если ОписаниеПомещенногоФайла <> Неопределено И ОписаниеПомещенногоФайла.ПомещениеФайлаОтменено = Ложь
		И ОписаниеПомещенногоФайла.СсылкаНаФайл.Файл <> Неопределено Тогда
		ПроверитьИВложитьФайл(ОписаниеПомещенногоФайла.СсылкаНаФайл.Файл.ПолноеИмя, ОписаниеПомещенногоФайла.Адрес);
	КонецЕсли;
