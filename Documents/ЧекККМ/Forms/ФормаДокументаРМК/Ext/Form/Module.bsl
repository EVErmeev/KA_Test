...
Процедура ОтменитьОплатуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОтменитьОплатуЗавершениеЛокализация();
...
Процедура НастроитьРМК()
	...
	Элементы.ОплатитьПодарочнымСертификатом.Доступность = Истина;
...
Процедура НастроитьРМКЛокализация(ДоступныеВидыОплаты, ЭлементыНаПерегруппировку)
	...
	Элементы.ОплатитьЭСФСС.Доступность = ЕстьТоварыСВозможностьюОплатыЭС;
	Элементы.СмешаннаяОплата.Доступность = Истина;
...
&НаКлиенте
Процедура ОтменитьОплатуЗавершениеЛокализация()
	
	Если ИнформацияОбОплате(ЭтотОбъект).СБП > 0 Тогда
		ОтложитьНаКлиенте(Неопределено);
		НастроитьРМК();
	КонецЕсли;
