	...
	Если ЗначениеЗаполнено(Объект.Ссылка) = Ложь И ОбщегоНазначенияКлиент.ДатаСеанса() < Дата('20220917') Тогда
		...
		ПоказатьВопрос(ВопросЗавершение, 
				ОбменСГИСЭПДКлиент.ТекстВопросаОПоддержкеОператоромЭПД(
					ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.ЭЗН"), Истина), 
...
Процедура ЗаполнитьИдентификациюФайлаОбмена(ИнформацияПоПрефиксамТитула, ИдентификаторОтправителя, Переформировать = Ложь)
	...
	Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул1")
		И НЕ ЗначениеЗаполнено(Объект.УИДМинтранс) Тогда
		ПолучитьГУИДОтОператора(Объект.ИдентификаторФрахтователя);
	ИначеЕсли Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭЗН_Титул2")
		И НЕ ЗначениеЗаполнено(Объект.УИДМинтранс) Тогда
		ПолучитьГУИДОтОператора(Объект.ИдентификаторФрахтователя, Истина);	
	КонецЕсли;
