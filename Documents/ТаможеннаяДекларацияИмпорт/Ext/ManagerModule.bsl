...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|		КОГДА &ПартионныйУчетВерсии22
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(Суммы.СуммаНДСРегл, &КоэффициентПересчетаВВалютуРегл * ТаблицаВидыЗапасов.СуммаНДС)
	|	КОНЕЦ 									КАК СтоимостьРегл,
	|	0										КАК НДСРегл,
	|	0 										КАК СтоимостьУпр,
	|	0		 								КАК НДСУпр,
	...
	|		  ИНАЧЕ ЕСТЬNULL(Суммы.СуммаНДСРегл, &КоэффициентПересчетаВВалютуРегл * ТаблицаВидыЗапасов.СуммаНДС)
	|	КОНЕЦ 									КАК ДопРасходыРегл,
	|	ЕСТЬNULL(Суммы.СуммаНДСУпр, &КоэффициентПересчетаВВалютуУпр * ТаблицаВидыЗапасов.СуммаНДС)	КАК ДопРасходыУпр,
	|	0 										КАК ПостояннаяРазница,
	|	0 										КАК ВременнаяРазница,
	|	0										КАК КорСтоимость,
	|
