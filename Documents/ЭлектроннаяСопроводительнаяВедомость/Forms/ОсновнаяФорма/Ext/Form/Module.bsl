...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) = Ложь И ОбщегоНазначенияКлиент.ДатаСеанса() < Дата('20220917') Тогда
		ВопросЗавершение = Новый ОписаниеОповещения("ВопросОФормированииЭПД_Завершение", ЭтотОбъект);
		ПоказатьВопрос(ВопросЗавершение, 
				ОбменСГИСЭПДКлиент.ТекстВопросаОПоддержкеОператоромЭПД(
					ПредопределенноеЗначение("Перечисление.ТипыДокументовЭДО.ЭСВ"), Истина), 
...
&НаКлиенте
Процедура ПолучитьГУИДОтОператора(ИдентификаторАбонента)
	
	Объект.УИДМинтранс = ОбменСГИСЭПДВызовСервера.ПолучитьИдентификаторЧерезСервисОператораЭДО(ИдентификаторАбонента);
...
Процедура ЗаполнитьИдентификациюФайлаОбмена(ИнформацияПоПрефиксамТитула, ИдентификаторОтправителя, Переформировать = Ложь)
	...
	Если (Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСВ_Титул1_1")
			Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСВ_Титул1_2")
			Или Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭСВ_Титул1_5"))
		И НЕ ЗначениеЗаполнено(Объект.УИДМинтранс) Тогда
		ПолучитьГУИДОтОператора(Объект.ИдентификаторПеревозчика);
	КонецЕсли;
...
Процедура ОтправитьЧерезЭДОПослеЗаписи(Повторно = Ложь)
	...
	Если Повторно = Истина 
		//Или (ДанныеСостоянияЭДО.ПредставлениеСостояния = "Не начат" Или ДанныеСостоянияЭДО.ПредставлениеСостояния = "Не получен") Тогда
