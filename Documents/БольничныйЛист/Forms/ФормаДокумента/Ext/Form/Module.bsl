...
Процедура ЗаполнитьУсловияИсчисления()
	...
	Если ЗначениеЗаполнено(КадровыеДанныеСотрудника.ДатаУвольнения)
		И Объект.ДатаНачалаСобытия > КадровыеДанныеСотрудника.ДатаУвольнения Тогда
...
Функция КадровыеДанныеСотрудника()
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Объект.Сотрудник, "ДатаУвольнения,Инвалидность", Объект.ДатаНачалаСобытия);
...
Процедура ОбновитьЭлементыСЭДО()
	...
	// СЭДО с 2022 года.
	Если ЗначениеЗаполнено(Объект.ДатаНачалаСобытия)
		И Объект.ДатаНачалаСобытия < СЭДОФССРасширенный.ДатаНачалаПроактивныхВыплат() Тогда
		Элементы.СЭДОГруппа.Видимость = Ложь;
		Возврат;
	КонецЕсли;
