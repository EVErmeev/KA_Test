...
Функция ТекстЗапросаВтДанныеРасходы(Запрос, ТекстыЗапроса) Экспорт
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(ДанныеДокументаРасходы.Сумма * ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(31, 2))
	|	КОНЕЦ                                      КАК Сумма,
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация В (
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеПрочихРасходов),
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(ДанныеДокументаРасходы.СуммаБезНДС * ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(31, 2))
	|	КОНЕЦ КАК СуммаБезНДС,
...
Функция ТекстЗапросаВтДанныеДоходы(Запрос, ТекстыЗапроса) Экспорт
	...
	//-- НЕ УТ
	|
	|	ВЫРАЗИТЬ(ДанныеДокументаДоходы.Сумма * ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(31, 2)) КАК Сумма,
	|	ВЫБОР
	|		КОГДА НЕ &УправленческийУчетОрганизаций
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ДанныеДокументаДоходы.СуммаУпр * ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(31, 2))
	|	КОНЕЦ КАК СуммаУпр,
	|	ВЫБОР
