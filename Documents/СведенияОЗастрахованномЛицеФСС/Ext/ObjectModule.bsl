...
Процедура ОбработкаЗаполнения(СсылкаОснования, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(СсылкаОснования) = Тип("ДокументСсылка.Увольнение") Тогда
		...
		Организация = РеквизитыОснования.Организация;
		Сотрудник   = РеквизитыОснования.Сотрудник;
	ИначеЕсли ТипЗнч(СсылкаОснования) = Тип("СправочникСсылка.Сотрудники") Тогда
		Сотрудник = СсылкаОснования;
		ОбновитьВторичныеДанные(Неопределено);
	КонецЕсли;
КонецПроцедуры
...
Функция КадровыеДанныеСотрудника()
	...
	Если ДополнительныеСвойства.Свойство("КадровыеДанные", КадровыеДанныеСотрудника) Тогда
		Возврат КадровыеДанныеСотрудника;
	КонецЕсли;
	
	ИменаПолей = Документы.СведенияОЗастрахованномЛицеФСС.ИменаПолейТребуемыхКадровыхДанных();
	КадровыеДанныеСотрудника = КадровыйУчет.КадровыеДанныеСотрудника(Истина, Сотрудник, ИменаПолей, ТекущаяДатаСеанса());
	ДополнительныеСвойства.Вставить("КадровыеДанные", КадровыеДанныеСотрудника);
	Возврат КадровыеДанныеСотрудника;
КонецФункции
