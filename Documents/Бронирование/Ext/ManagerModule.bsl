...
Функция ТекстЗапросаТаблицаДенежныеСредстваУПодотчетныхЛиц(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеДокумента.Сумма * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр, &КоэффициентПересчетаВВалютуУпр) КАК СуммаУпр,
	|	ДанныеДокумента.Сумма * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл, &КоэффициентПересчетаВВалютуРегл) КАК СуммаРегл,
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваКонтрагент(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|	ДанныеДокумента.Сумма * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр, &КоэффициентПересчетаВВалютуУпр) КАК СуммаОплаты,
	|	ДанныеДокумента.Сумма * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл, &КоэффициентПересчетаВВалютуРегл) КАК СуммаОплатыРегл,
...
Функция ТекстЗапросаТаблицаВтСуммыДокументовВВалютахУчета(Запрос, ТекстыЗапроса)
	...
	|	(ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС) * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл, &КоэффициентПересчетаВВалютуРегл) КАК СуммаБезНДСРегл,
	|	ТаблицаДокумента.СуммаНДС * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл, &КоэффициентПересчетаВВалютуРегл) КАК СуммаНДСРегл,
	|	НЕОПРЕДЕЛЕНО КАК ТипРасчетов,
	|	(ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС) * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр, &КоэффициентПересчетаВВалютуУпр) КАК СуммаБезНДСУпр,
	|	ТаблицаДокумента.СуммаНДС * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр, &КоэффициентПересчетаВВалютуУпр) КАК СуммаНДСУпр,
	|
	|	ЛОЖЬ КАК ОтражаетсяВРасчетах,
	|	ТаблицаДокумента.Сумма КАК СуммаСНДС,
	|	&Период КАК ПериодБазыНДС,
	|	НЕОПРЕДЕЛЕНО КАК ОбъектРасчетов,
	|	(ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНеОблагаемаяНДС - ТаблицаДокумента.СуммаНДС) * ЕСТЬNULL(ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл, &КоэффициентПересчетаВВалютуРегл) КАК БазаНДСРегл,
	...
	|	НЕОПРЕДЕЛЕНО КАК ТипРасчетов,
	|	(ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС) * &КоэффициентПересчетаВВалютуУпр КАК СуммаБезНДСУпр,
	...
	|	&Период КАК ПериодБазыНДС,
	|	НЕОПРЕДЕЛЕНО КАК ОбъектРасчетов,
	|	(ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНеОблагаемаяНДС - ТаблицаДокумента.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК БазаНДСРегл,
	|	ТаблицаДокумента.Ссылка.Валюта КАК ВалютаВзаиморасчетов,
	|	ТаблицаДокумента.Сумма КАК СуммаВзаиморасчетов,
	|	0 КАК СуммаНДСВзаиморасчетов
	|ИЗ
	...
	|	0 КАК СуммаНДС,
	|	ТаблицаДокумента.СуммаШтрафа * &КоэффициентПересчетаВВалютуРегл КАК СуммаБезНДСРегл,
	|	0 КАК СуммаНДСРегл,
	|	НЕОПРЕДЕЛЕНО КАК ТипРасчетов,
	|	ТаблицаДокумента.СуммаШтрафа * &КоэффициентПересчетаВВалютуУпр КАК СуммаБезНДСУпр,
	|	0 КАК СуммаНДСУпр,
	|
	|	ЛОЖЬ КАК ОтражаетсяВРасчетах,
	...
	|	&Период КАК ПериодБазыНДС,
	|	НЕОПРЕДЕЛЕНО КАК ОбъектРасчетов,
	|	ТаблицаДокумента.СуммаШтрафа * &КоэффициентПересчетаВВалютуРегл КАК БазаНДСРегл,
	|	ТаблицаДокумента.Ссылка.Валюта КАК ВалютаВзаиморасчетов,
