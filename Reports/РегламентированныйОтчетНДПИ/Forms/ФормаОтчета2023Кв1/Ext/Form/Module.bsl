...
Процедура Проверить(Команда)
	...
	"ru='Проверка контрольных соотношений в форме отчета,
	|утвержденной приказом ФНС России от 20.09.2022 № ЕД-7-3/854@,
	...
	Для каждого СтрокаТЗ Из ТаблицаСписков Цикл
		Если НЕ МассивПолейСписочныхЗначений.Найти(СтрокаТЗ.Показатель) = Неопределено Тогда
			...
			ПреобразоватьВЧисло = СтрокаТЗ.Показатель = "П000200023003"
				ИЛИ СтрокаТЗ.Показатель = "П000300011003" ИЛИ СтрокаТЗ.Показатель = "П000300017003" ИЛИ СтрокаТЗ.Показатель = "П000500005003";
...
Процедура ПроверкаКС_1_03()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru='Сумма налога в стр.030 Раздела 1 = сумма стр.150 Раздела 2 или '") +
		...
		НСтр("ru='стр.150 подр.7.2 Раздела 7 не заполнена, или стр.150 подр.7.2 Раздела 7 или стр.180 подр.8.1 Раздела '") +
...
Процедура ПроверкаКС_8_18()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел8 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел81 Цикл
			...
			Настройки.ЭтоОшибка = НЕ Показатели.Р81С180.Значение = РасчетнаяСумма;
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р81С180.Значение, Показатели.Р81С130.Значение, Показатели.Р81С135.Значение, РасчетнаяСумма);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.НомераСтраницРазделов5СКодамиВидаДПИ04104_10017 = Новый Массив;
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("НомераСтраницРазделов5СКодамиВидаДПИ04104_10017");
		...
		Для каждого СтраницаРаздела5 Из ДанныеРаздела5.Строки Цикл
			...
			Для каждого СтраницаРаздела51 Из СтраницаРаздела5.Строки Цикл
				...
				Пока СтраницаРаздела51.Данные.Свойство("П000500005501_" + НомерСтроки51Стр) Цикл
					...
					ИначеЕсли (ПрКоэф51 = "05"
						   ИЛИ ПрКоэф51 = "07")
						   И НЕ (ЗначКоэф51= 0.1
						     ИЛИ ЗначКоэф51= 1) Тогда
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	Если ИмяТекРаздела = "Раздел51" Тогда
		ОбновитьНомераСтраницРазделов5СКодамиВидаДПИ04104_10017();
	КонецЕсли;
			...
			Для каждого ВеткаРаздел51 Из ВеткаРаздел5.Строки Цикл
				...
				Если Форма.СтруктураРеквизитовФормы["ПрименяетсяПисьмоФНС_20221012_СД_4_3_13613"] Тогда
					...
					Если НЕ СокрЛП(ВеткаРаздел5.Данные["П000500001003"]) = "10017"
					   И НЕ СокрЛП(ВеткаРаздел5.Данные["П000500001003"]) = "04104" Тогда
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "П000500005502") > 0
		...
		Если ПризнакКоэфф = "05"
		 ИЛИ ПризнакКоэфф = "07" Тогда
...
&НаСервере
Процедура ОбновитьНомераСтраницРазделов5СКодамиВидаДПИ04104_10017()
	
	ДеревоДанныхРаздела = ПолучитьИзВременногоХранилища(
		СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРаздел5"]);
	
	СтруктураРеквизитовФормы.НомераСтраницРазделов5СКодамиВидаДПИ04104_10017.Очистить();
	
	Для каждого Ветка5 Из ДеревоДанныхРаздела.Строки Цикл
		Если СокрЛП(Ветка5.Данные["П000500001003"]) = "10017"
		 ИЛИ СокрЛП(Ветка5.Данные["П000500001003"]) = "04104" Тогда
			СтруктураРеквизитовФормы.НомераСтраницРазделов5СКодамиВидаДПИ04104_10017.Добавить(
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел51" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы["ПрименяетсяПисьмоФНС_20221012_СД_4_3_13613"] Тогда
			...
			Если Форма.СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет
			 ИЛИ НЕ Форма.СтруктураРеквизитовФормы.НомераСтраницРазделов5СКодамиВидаДПИ04104_10017.Найти(
					Форма.СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела[0]) = Неопределено Тогда
				ОбрабатываемаяОбласть.Защита   = Ложь;
				ОбрабатываемаяОбласть.ЦветФона = Форма.мСтруктураЦветовРаскраски["_0"];
			Иначе
