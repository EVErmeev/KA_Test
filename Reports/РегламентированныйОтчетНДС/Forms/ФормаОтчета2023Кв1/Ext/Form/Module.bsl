...
Процедура ПроверкаКС_1_09()
	...
	НастройкиКС.ЭтоОшибка = РасчетнаяСумма >= 0 И Показатели.Р03С200Г3.Значение < РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_21_код()
	...
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
		...
		РасчетнаяСумма);
	НастройкиКС.ЭтоОшибка = ?(НастройкиКС.КлючИтога = "Р01С040", 
		ПоказателиИтога[НастройкиКС.КлючИтога].Итог < РасчетнаяСумма,
		ПоказателиИтога[НастройкиКС.КлючИтога].Итог > РасчетнаяСумма);
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_1_24()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='(возможно влияние погрешности округления)'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибочная сумма НДС, подлежащая уплате в бюджет'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму НДС в стр.030 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru='п.5 ст.173 НК РФ'"));
	Заголовки.Вставить("Комментарий"		, 
		НСтр("ru='Важно! В случае данной ошибки декларация по НДС может считаться непредставленной!'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р01С030.Значение) И Показатели.Р01С030.Значение < 
...
Процедура ПроверкаКС_1_26а()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='(стр.280 + стр.285 + стр.290 Приложения 1 к Разделу 9 в которых стр.090 = 06)'"));
	...
	НастройкиКС.ЭтоОшибка = Код1011715Отсутствует И Показатели.Р02С060.Итог < 
...
Процедура ПроверкаКС_1_26б()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='которых стр.010 = 41, 43, 44)'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %5 (%2 - %3 + %4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибочная сумма НДС, подлежащая уплате в бюджет'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму НДС в стр.060 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru='ст.161, п.4, 4.1 ст.173 НК РФ'"));
	...
	НастройкиКС.ЭтоОшибка = Код1011715Присутствует И ПоказателиР02.Р02С060.Итог < 
		РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, РасчетнаяСумма);
...
Процедура ПроверкаКС_1_27()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='Приложения 1 к Разделу 9, с учетом погрешности округления'"));
	...
	Заголовки.Вставить("Основание"			, НСтр("ru='ст.153, 161, 164, 165, 166, 167, 173 НК РФ'"));
	Заголовки.Вставить("Комментарий"		, 
		НСтр("ru='Важно! В случае данной ошибки декларация по НДС может считаться непредставленной!'"));
	ПодготовитьНастройки(Заголовки);
	...
	ШаблонДетализацииРазделов	= НСтр("ru='%1 + %2 + %3 + %4 + %5 + %6 + 0,49 х 6'");
	...
	РасчетнаяСуммаРаздОкр 	= РасчетнаяСуммаРазделов + 0.49 * 6;
	...
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
		РасчетнаяСуммаРаздОкр, РасшифровкаРазделов, РасчетнаяСуммаКнигПрод, РасшифровкаКнигПрод);
	НастройкиКС.ЭтоОшибка = РасчетнаяСуммаРаздОкр < РасчетнаяСуммаКнигПрод;
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_1_28()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='Разделу 8 и стр.005 Приложения 1 к Разделу 8, с учетом погрешности округления'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 (%2) <= %3 (%4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибочная сумма НДС, подлежащая вычету'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните показатели для соблюдения равенства");
	Заголовки.Вставить("Основание"			, НСтр("ru='ст.171, 172 НК РФ'"));
	...
	ШаблонДетализацииРазделов	= НСтр("ru='%1 + %2 + %3 + %4 + %5 + %6 +%7 + %8 - 0,49 х 8'");
	...
	РасчетнаяСуммаРаздОкр 	= РасчетнаяСуммаРазделов - 0.49 * 8;
...
Процедура ПроверкаКС_1_29()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='Раздела 8 = 20 + стр.180 Приложения 1 к Разделу 8, в которых стр.010 = 20)'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <= %4 (%2 + %3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибочная сумма НДС, подлежащая вычету'"));
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р03С150.Итог > РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_30()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='Приложения 1 к Разделу 8, в которых стр.010 = 19)'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <= %4 (%2 + %3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибочная сумма НДС, подлежащая вычету'"));
...
Процедура ПроверкаКС_1_31()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='стр.010 Раздела 8 = 06 + стр.180 Приложения 1 к Разделу 8, в которых стр.010 = 06)'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <= %4 (%2 + %3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибочная сумма НДС, подлежащая вычету'"));
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р03С180.Итог > РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_32()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р08С190.Значение > Показатели.Р08С180.Значение;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_33()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р81С190.Значение > РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_37()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р09С260.Значение < Показатели.Р09С200.Значение;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_38()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р09С270.Значение < Показатели.Р09С210.Значение;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_42()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р91С340.Значение < РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_43()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р91С350.Значение < РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_46()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р09С265.Значение < Показатели.Р09С205.Значение;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_48()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р91С345.Значение < РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
...
Процедура ПроверкаКС_1_51()
	...
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 1011703'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно указан неверный код'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните код в стр.070 Раздела 2");
