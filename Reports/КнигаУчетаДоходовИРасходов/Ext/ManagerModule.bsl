...
Процедура ПечатьРаздела5(ПараметрыОтчета)
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(ВыборкаЗаписей.ДатаНомер) Тогда
			...
			ПараметрыПервичногоДокумента = УчетУСНПСНСервер.ПараметрыПервичногоДокумента();
			...
			Секция.Параметры.ДатаНомер = УчетУСНПСНСервер.РеквизитыПервичногоДокумента(ВыборкаЗаписей.Регистратор, ПараметрыПервичногоДокумента);
		КонецЕсли;
...
Процедура ВывестиУменьшенияНалогаЗаКвартал(ПараметрыОтчета, ТекКвартал, ПоследнийКварталПериода, Макет, ТабличныйДокумент)
	...
	Секция = Макет.ПолучитьОбласть("Итог");
	РимскоеЧисло = ТекКвартал;
	Секция.Параметры.ПредставлениеПериодаИтогов = СтрШаблон(НСтр("ru = 'Итого за %1 квартал'"),
		СтроковыеФункцииКлиентСервер.ПреобразоватьЧислоВРимскуюНотацию(РимскоеЧисло, Ложь));
...
Функция ТекстЗапросаПодсчетСтрокРаздела4ДоТекущегоКвартала(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
	...
	Если ПараметрыОтчета.КонецПериода < Дата(2013, 1, 1) Тогда
