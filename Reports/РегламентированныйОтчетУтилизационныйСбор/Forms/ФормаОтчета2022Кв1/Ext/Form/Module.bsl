		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если СтрРазделить("M1,M1G", ",", Ложь).Найти(КатегорияТС) <> Неопределено Тогда
					ТДО["П000030014003"].Значение = 20000;
				ИначеЕсли ЗначениеЗаполнено(КатегорияТС) Тогда
					ТДО["П000030014003"].Значение = 150000;
				Иначе
					ТДО["П000030014003"].Значение = 0;
				КонецЕсли;
	...
	Если ИмяРаздела = "Раздел3" Тогда
		...
		Для каждого Ветка_2 Из ДеревоДанных.Строки Цикл
			...
			Для каждого Ветка Из Ветка_2.Строки[0].Строки Цикл
				...
				// Стр.140.
				КатегорияТС = СокрЛП(Данные["П000030009003"]);
				Если СтрРазделить("M1,M1G", ",", Ложь).Найти(КатегорияТС) <> Неопределено Тогда
					Данные.Вставить("П000030014003", 20000);
				ИначеЕсли ЗначениеЗаполнено(КатегорияТС) Тогда
					Данные.Вставить("П000030014003", 150000);
				Иначе
					Данные.Вставить("П000030014003", 0);
				КонецЕсли;
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Для каждого ПодчВетка Из Ветка.Строки[0].Строки Цикл
				...
				Иначе
					...
					Если СтрРазделить(СтрокаПроверки, ", ", Ложь).Найти(КатегорияТС) <> Неопределено И ЗначПок <> 20000 Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
						+ ": базовая ставка должна быть равна 20 000 рублей, если категория (вида) транспортного средства "
						+ "равна M1 или M1G", "Раздел3", ИмяПок, КоординатыПодчВетки);
						СтатусПроверки = Ложь;
					ИначеЕсли СтрРазделить(СтрокаПроверки, ", ", Ложь).Найти(КатегорияТС) = Неопределено И ЗначПок <> 150000 Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
						+ ": базовая ставка должна быть равна 150 000 рублей, если категория (вида) транспортного средства "
				...
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					...
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ((ДлинаЗначПок = 10 И СтрДлина(СтрЗаменить(ЗначПок, " ", "")) = 10)
					ИЛИ (ДлинаЗначПок = 15 И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЗначПок, , Истина))) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": серия, номер паспорта представляются в виде серии (4 знака) и номера паспорта (6 знаков) без пробелов "
					+ "или в виде 15 цифр в соответствии с решением Коллегии Евразийской экономической комиссии от 22.09.2015 № 122 "
					+ """Об утверждении Порядка функционирования систем электронных паспортов транспортных средств (электронных "
					+ "паспортов шасси транспортных средств) и электронных паспортов самоходных машин и других видов техники""",
					"Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ПроверкаКС_2_03()
	...
	Заголовки.Вставить("Описание"			, "Значение базовой ставки в стр.140 Раздела 3 = 150.000, если признак " +
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			...
			НастройкиКС.ЭтоОшибка = НЕ МассивЗначений.Найти(Показатели.Р03С040.Значение) = Неопределено
				И НЕ Показатели.Р03С140.Значение = 150000;
...
Процедура ПроверкаКС_2_07()
	...
	Заголовки.Вставить("Описание"			, "Значение базовой ставки в стр.140 Раздела 3 = 20.000, 150.000");
	Заголовки.Вставить("Нарушение"			, "Ошибочное значение базовой ставки");
	Заголовки.Вставить("Основание"			, "ст.80 НК РФ");
	Заголовки.Вставить("Рекомендации"		, "Уточните значение базовой ставки");
	Заголовки.Вставить("ШаблонДетализации"	, "недопустимое значение %1");
