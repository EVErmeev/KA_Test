...
Функция НалоговаяСтавкаВидаПодакцизногоТовара(КодВидаПодакцизногоТовара, ДатаДействия)
	...
	ИначеЕсли ГодДействия = 2024 Тогда
		...
		СтавкаВидаПТ = ?(КодВидаПодакцизногоТовара = "471", 21,   СтавкаВидаПТ);
		СтавкаВидаПТ = ?(КодВидаПодакцизногоТовара = "472", 9016, СтавкаВидаПТ);
	...
	ИначеЕсли ГодДействия = 2025 Тогда
		...
		СтавкаВидаПТ = ?(КодВидаПодакцизногоТовара = "471", 22,   СтавкаВидаПТ);
		СтавкаВидаПТ = ?(КодВидаПодакцизногоТовара = "472", 9377, СтавкаВидаПТ);
...
Процедура СформироватьСлужебныеДанные()
	...
	ИначеЕсли ДанныеКС.ГодОтчета = 2025 Тогда
		...
		СтавкиСигарет.Вставить("СтавкаТвердая"		, 2815);
		СтавкиСигарет.Вставить("СтавкаАдвалорная"	, 16);
		СтавкиСигарет.Вставить("СтавкаМинимальная"	, 3825);
		
	ИначеЕсли ДанныеКС.ГодОтчета = 2024 Тогда
		
		СтавкиСигарет.Вставить("СтавкаТвердая"		, 2707);
		СтавкиСигарет.Вставить("СтавкаАдвалорная"	, 16);
		СтавкиСигарет.Вставить("СтавкаМинимальная"	, 3678);
...
Процедура ПроверкаКС_2_32_код()
	...
	Пока ВыборкаР1.Следующий() Цикл
		...
		Выборка = ВыборкаР1.Выбрать();
		Пока Выборка.Следующий() Цикл
			ЗаполнитьИРасшифроватьПоказатель(Показатели[НастройкиКС.ПоказательР1], Выборка);
		КонецЦикла;
...
Процедура ПроверкаКС_4_03()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Приложение2 Цикл
		...
		НастройкиКС.ЭтоОшибка = Показатели.П02С020.Значение > Показатели.П02С030.Значение;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, Показатели.П02С030.ЗначениеСтр, 
