...
Процедура СформироватьТитульныйЛист(ПараметрыОтчета)
	...
	Если Не Справочники.Патенты.ЕстьДействующийПатент(ПараметрыОтчета.Организация, , ПараметрыОтчета.КонецПериода) Тогда
	...
	Макет.Параметры.СрокПатента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Формат(ПараметрыОтчета.НачалоПериода, "ДЛФ=DD"),
		Формат(ПараметрыОтчета.КонецПериода, "ДЛФ=DD"));
	...
	Если ВыборкаОбщие.Следующий() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.КоличествоЗаписей = 1 Тогда
				ОбластьСтрока.Параметры.ТекРасшифровка.Вставить("НомерСтроки");
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(ОбластьСтрока.Параметры.ТекРасшифровка, Выборка);
			Если ЗначениеЗаполнено(Выборка.СторнирующийДокумент) Тогда
				ОбластьСтрока.Параметры.ТекРасшифровка.Регистратор = Выборка.СторнирующийДокумент;
			КонецЕсли;
