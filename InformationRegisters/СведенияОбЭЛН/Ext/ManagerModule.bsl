...
Процедура ЗагрузитьСообщение111(Страхователь, ИдентификаторСообщения, ТекстXML, Результат, Кэш) Экспорт
	...
	Если Не ДанныеЭЛН.Успех Тогда
		СЭДОФСС.ОшибкаОбработки(Результат, ИдентификаторСообщения, ДанныеЭЛН.ТекстОшибки, ТекстXML);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьСведенияОбЭЛН(Страхователь, ТекстXML, ДанныеЭЛН) Экспорт
	...
	Если Не ЗаполнитьБольничный Тогда
		ЗаполнитьБольничный = Не ЗначениеЗаполнено(Запись.Организация)
			Или СЭДОФСС.СтраховательОрганизации(Запись.Организация) <> Страхователь;
	КонецЕсли;
	Если Не ЗаполнитьБольничный Тогда
		ЗаполнитьБольничный = Не ЗначениеЗаполнено(Запись.Сотрудник)
