...
Процедура ЗагрузитьСообщение112(Страхователь, РегистрацияИдентификатор, ТекстXML, Результат) Экспорт
	// Пример:
	...
	КонецЕсли;
	Если КореньDOM = Неопределено Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'В xml-содержимом сообщения 112 не удалось найти узел ""eventLifeResult"". Текст XML: %1'"), ТекстXML);
	...
	Если ДоставкаИдентификатор = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'В xml-содержимом сообщения 112 отсутствует атрибут ""responseOn"".
			|Текст XML:
			|%1'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(ДоставкаИдентификатор) Тогда
		ТекстОшибки = НСтр("ru = 'В xml-содержимом сообщения 112 не заполнен атрибут ""responseOn"".
			|Текст XML:
			|%1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, ТекстXML);
		СЭДОФСС.ОшибкаОбработки(Результат, РегистрацияИдентификатор, ТекстОшибки);
	...
	Если Не ЗначениеЗаполнено(СведенияОСтатусе.Вес) Тогда
		...
		СЭДОФСС.ОшибкаОбработки(Результат, РегистрацияИдентификатор, РегистрацияПротокол);
	КонецЕсли;
...
Функция ПредставлениеСпискаОшибок(СписокОшибок)
	// Для сообщения 112 ожидаемые поля: code,message,details.
