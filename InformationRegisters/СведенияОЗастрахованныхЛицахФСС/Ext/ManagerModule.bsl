...
Функция ТребованияПоОтправке()
	...
	НачалоАктуальности = ДобавитьМесяц(НачалоТекущегоДня, -1);
	...
	|ГДЕ
	|	СведенияОЗастрахованныхЛицахФСС.МаксимальнаяДатаОтправки >= &НачалоАктуальности
...
Процедура ОбновитьСведения(ГоловнаяОрганизация, Знач ФизическиеЛица) Экспорт
	...
	РегистрыСведений.ОчередьОбработкиКадровыхДанныхФСС.ЗапланироватьОбновлениеПоГоловнойОрганизации(
		ГоловнаяОрганизация,
		ФизическиеЛица);
КонецПроцедуры

// Добавляет в регистр информацию о новых сотрудниках, которые приняты после даты вступления закона в силу.
Процедура ДобавитьСведенияОНовыхСотрудниках(ГоловнаяОрганизация, ФизическиеЛица) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
