...
Процедура ЗагрузитьСообщение106(Страхователь, РегистрацияИдентификатор, ТекстXML, Результат) Экспорт
	...
	КонецЕсли;
	Если КореньDOM = Неопределено Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'В xml-содержимом сообщения 106 не удалось найти узел ""noticeTerminationResponse"". Текст XML: %1'"), ТекстXML);
	...
	Если ИдентификаторСообщения = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'В xml-содержимом сообщения 106 отсутствует тег ""recriveID"".
			|Текст XML:
			|%1'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(ИдентификаторСообщения) Тогда
		ТекстОшибки = НСтр("ru = 'В xml-содержимом сообщения 106 не заполнен тег ""recriveID"".
			|Текст XML:
			|%1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, ТекстXML);
		СЭДОФСС.ОшибкаОбработки(Результат, РегистрацияИдентификатор, ТекстОшибки);
