...
&НаСервере
Функция ДобавитьПодобранныеТовары(ПодобранныеТовары, СтрокиТовары, Упаковка, Цена, КоэффициентУпаковки, ТипНоменклатуры = Неопределено)
...
Функция ПодобранныеТовары()
	...
	Иначе
		...
		ХарактеристикиИспользуются = ЗаполнитьИспользованиеХарактеристик(ВсеКомплектующие);
		...
		Для Каждого ПозицияНабора Из Объект.СоставНабора Цикл
			...
			Если Подобрано > 0 Тогда
				...
				ПараметрыТовара.ВариантОбеспечения = Объект.ВариантОбеспечения;
				ПодобранныеТовары.Добавить(ПараметрыТовара);
				ПараметрыТовара.ХарактеристикиИспользуются = ХарактеристикиИспользуются.Получить(ПозицияНабора.Номенклатура);
...
Функция ЗаполнитьИспользованиеХарактеристик(ВсеКомплектующие)
	...
	ХарактеристикиИспользуются = Новый Соответствие;
	...
	Пока Выборка.Следующий() Цикл
		ХарактеристикиИспользуются.Вставить(Выборка.Номенклатура, Выборка.ХарактеристикиИспользуются);
	КонецЦикла;
