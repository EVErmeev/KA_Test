...
Процедура РассчитатьИтоговыеСуммы2_5(ИтоговыеСуммы, СтрокаТовары, ПрослеживаемыеТовары = Неопределено, ОблагаетсяНДССМежценовойРазницы = Ложь)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			Если ДействуетПостановление534(ДанныеПечати.Дата, ДанныеПечати.ДатаИсправления) Тогда
				...
				Если ИспользоватьПечатныеФормыСФ2_5 Тогда
					ОпцииПечати.ИспользоватьПечатныеФормыСФ2_5 = ИспользоватьПечатныеФормыСФ2_5;
					Макет = ?(ДанныеПечати.ЕстьПрослеживаемыеТовары Или ЕстьПрослеживаемыеТоварыВТабличнойЧасти, МакетУКД_534Прослеж_2_5, МакетУКД_534_2_5);
				Иначе
					Макет = МакетУКД_534;
				КонецЕсли;
				ЕстьПостановление534 = Истина;
