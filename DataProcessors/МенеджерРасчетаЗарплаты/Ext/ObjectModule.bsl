...
Процедура ЗаполнитьЗначенияПериодическихПоказателейСотрудников(ПоказателиСотрудников, СтрокиНачисленийПоИдентификаторам)
	...
	ЗначенияПериодическихПоказателейСотрудников = ЗначенияПериодическихПоказателейСотрудников();
	ЗначенияПериодическихПоказателейСотрудников.Сортировать("Сотрудник, Показатель, ДатаНачала, ПериодЗаписи");
		...
		Для Каждого КлючИЗначение Из СтрокиПоИдентификатору Цикл 
			...
			Для Каждого СтрокаРасшифровки Из Строка.Дополнительно.РасшифровкаРасчетнойБазы Цикл 
				...
				Если НайденныеСтроки.Количество() = 0 Тогда 
					...
					НоваяСтрока.Результат = СтрокаРасшифровки.Сумма;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Функция РезультатРасчетаНДФЛ()
	...
	Если НастройкиНДФЛ.ОкончательныйРасчет И НастройкиНДФЛ.ЗасчитыватьДанныеАвансов И СписокФизическихЛиц.Количество() > 0 И НаборыЗаписей.СведенияОДоходахНДФЛ.Количество() = 0 Тогда
		ПериодыРегистрацииСотрудников = Новый ТаблицаЗначений;
		ПериодыРегистрацииСотрудников.Колонки.Добавить("ПериодРегистрации", Новый ОписаниеТипов("Дата"));
		ПериодыРегистрацииСотрудников.Добавить().ПериодРегистрации = МесяцНачисления;
		УчетНДФЛ.ДополнитьДвиженияСписаниемАванса(ИсключаемыйРегистратор, НаборыЗаписей, Организация, КонецМесяца(МесяцНачисления), ПериодыРегистрацииСотрудников, Истина, СписокФизическихЛиц);
	КонецЕсли;
