...
Процедура ЗаполнитьКоличествоПоДокументуОСУ(Команда)
	...
	КонецЕсли;
	ВесаПоGTIN = ПроверкаИПодборПродукцииИСМПВызовСервера.ВесПоGTIN(АнализируемыеGTIN, ПараметрыСканирования);
	...
	Для Каждого СтрокаМаркируемойПродукции Из СтрокиЗаполнения Цикл
		...
		Если СтрокаМаркируемойПродукции.ПроизвольнаяЕдиницаУчета Тогда
			...
			КоличествоВложенныхЕдиниц = Окр(КоличествоКИзменению / Вес, 0);
			КоличествоПодобрано       = Окр(КоличествоВложенныхЕдиниц * Вес, 3);
