...
Процедура ТоварыПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Если ПараметрыОтображенияОстатковЦен.ОстатокПоУчету.ОтображатьВТаблице
		...
		Если Настройки.ДополнительныеСвойства.ИспользуетсяПроизводительныйРежим Тогда
			ТекстРеквизитаОстатка = "СУММА(ЕСТЬNULL(ИсточникОстатков.ВНаличииОстаток - ИсточникОстатков.РезервироватьНаСкладеОстаток - ИсточникОстатков.РезервироватьПоМереПоступленияОстаток, 0))";
			ТекстТаблицыОстатка = "
...
Процедура УстановитьДоступностьЭлементовДляСтатуса()
	...
	Элементы.ВернутьИзАрхива.Доступность = СтатусВАрхиве;
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Статус = "Выполнено" 
		...
		Если РезультатОперации.ЗначениеУстановлено Тогда
			ИнформацияПоСчетчикуТоваров = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Возможна публикация и обновление %1 позиций из доступных %2 позиций. Сброс счетчика будет выполнен %3'"),
				РезультатОперации.Остаток,
				РезультатОперации.Значение,
				РезультатОперации.ВремяСброса);
		Иначе
...
Процедура ДобавитьПолучениеЦеныВТекстЗапроса(СтруктураПараметров)
	...
	ТекстРеквизита = СтрШаблон(
	"	СУММА(ВЫБОР
	|			КОГДА ТоварыСЦенами.ВидЦены = &%1
	|					И ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковкиЦены, 0) <> 0
	|				ТОГДА ТоварыСЦенами.Цена / &ТекстЗапросаКоэффициентУпаковкиЦены * &ТекстЗапросаКоэффициентУпаковкиТовара
	|			КОГДА ТоварыСЦенами.ВидЦены = &%1
	|					И ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковкиЦены, 0) = 0
	|				ТОГДА ТоварыСЦенами.Цена
	|			ИНАЧЕ 0
	|		КОНЕЦ)", СтруктураПараметров.ИмяРеквизита);
...
Процедура УстановитьДоступностьВидимостьЭлементов()
	...
	Элементы.ВернутьИзАрхива.Доступность = СтрокаТаблицыВыбрана И УчетнаяЗаписьВыбрана;
